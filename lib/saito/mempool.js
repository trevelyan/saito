...
console.log("Adding Blockto Mempool!");
...
console.log(fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
	  ...
	  // we do not want to create a block with a minimal balance of 100 unless a sufficiently large
          // amount of time has passed since the previous block AND/OR the time that we loaded the program
	  // this delay is intended to stop applications from churning out blocks to make money while they
	  // are still loading the blockchain
	  //
	  // the negative consequences of failing to solve this are blockchain bloat that gets indexed and
	  // fed out to all subsequent clients. So we solve this by not letting us print money if we are
	  // connected to other full-node peers.
          if (mempool_self.app.wallet.returnBalance() < 100) {
            var tmptime = new Date().getTime();
console.log("X: "+mempool_self.blocks.length + " -- " + (tmptime-mempool_self.load_time));
            if (mempool_self.blocks.length > 0 || (tmptime-mempool_self.load_time) <= mempool_self.load_delay) { return; }
          }
