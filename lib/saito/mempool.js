...
console.log(tmpdate + ": " + fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
    ...
    this.processing_timer = setInterval(function() {
      ...
      if (blk != null) {
        ...
        if (blk.prevalidated == 0) {
          delete_blk_from_mempool = mempool_self.app.blockchain.indexAndStore(blk);
	} else {
          delete_blk_from_mempool = mempool_self.app.blockchain.indexAndStore(blk, "force");
	}
