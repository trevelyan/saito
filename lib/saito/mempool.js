...
function Mempool(app) {
  ...
  this.processing_speed           = 300; // 0.3 seconds
  ...
  if (tx.transaction.gt != null) {
    for (var z = 0; z < this.transactions.length; z++) {
      if (this.transactions[z].transaction.gt != null) {
        if (tx.returnUsableFee() > this.transactions[z].returnUsableFee() || (this.transactions[z].transaction.from[0].add != this.app.wallet.returnPublicKey() && tx.transaction.from[0].add == this.app.wallet.returnPublicKey())) {
...
console.log("about to validate inputs with callback on success!");
    this.app.storage.validateInputsWithCallbackOnSuccess(tx, function(app, tx) {
console.log(" ... successfully validated! importing!");
...
console.log("adding block:"+blk.returnId());
...
console.log(fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
	  ...
	  // we do not permit nodes to create blocks for money unless they are on a private network and/or
          // have no connections to the outside world. If the network runs out of money for some reason 
	  // because all users with money vanish and no-one has the cash to send a single transaction, then
	  // a node can pull away, generate a few blocks and rebroadcast them on rejoining the network.
          if (mempool_self.app.wallet.returnBalance() < 100) {
            var tmptime = new Date().getTime();
            if (mempool_self.blocks.length > 0 || mempool_self.app.network.returnFullNodePeersTotal() > 0) {
              return;
            } else {
              if (mempool_self.app.options.peers != null) {
                if (mempool_self.app.options.peers.length > 0) {
                  ...
                  if (mempool_self.app.options.server != null) {
                    if (mempool_self.app.options.peers.length == 1 && mempool_self.app.options.peers[0].host == mempool_self.app.options.server.host) {} else { return; }
                  } else {
                    return;
                  }
                }
...
console.log(fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
	  ...
	  // we do not createa  block unless we have 1 transaction (i.e. 
	  // at least a golden ticket). This will be for the longest
	  // chain by definition.
	  if (mempool_self.containsGoldenTicket() == 0 && latestBlk.block.id != 0) { return; }
...
console.log(nb);
console.log(" ... with this as our previous block ...");
console.log(prevblk);
...
console.log("B "+this.blocks.length);
...
console.log("X");
          mempool_self.app.blockchain.indexAndStore(blk);
	} else {
console.log("Y");
console.log(blk);
          mempool_self.app.blockchain.indexAndStore(blk, "force");
	}
console.log("Z");
        mempool_self.cleanup(blk);
      }

console.log("E: indexed and stored: "+mempool_self.blocks.length);
...
Mempool.prototype.cleanup = function cleanup(blk) {
  for (var bt = 0; bt < blk.transactions.length; bt++) {
...
console.log("ABOUT TO RETURN BLOCK: "+this.blocks.length);
console.log("eenurning block id: "+this.blocks[0].returnId());
  var tmpblk = this.blocks[0];
  this.blocks.splice(0,1);
console.log("returning block id: "+tmpblk.returnId());
  return tmpblk;
}
