...
Mempool.prototype.addTransaction = function addTransaction(tx, relay_on_validate=1) {
  ...
  // avoid adding twice
  if (this.containsTransaction(tx) == 1) { console.log("add mempool: 1"); return; }
  if (tx == null) 			 { console.log("add mempool: 2"); return; }
  if (tx.transaction == null) 		 { console.log("add mempool: 3"); return; }
...
console.log((new Date()) + ": " + fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
      ...
      if (parseFloat(fees_available) >= parseFloat(fees_needed) && ( (mempool_self.returnNormalTransactionsInMempool() > 0 || mempool_self.app.wallet.returnBalance() < 101) || mempool_self.app.blockchain.index.hash.length == 0)) {
	...
	// do not create unless we have 1 transaction
	  ...
	  if (mempool_self.blocks.length == 0) {
	    ...
	    //if (mempool_self.containsGoldenTicket() == 1) {
	      //if (latestBlk.block.id < 3) {
