...
Mempool.prototype.addTransaction = function addTransaction(tx) {
  ...
  if (tx.transaction.gt != null) {
    for (z = 0; z < this.transactions.length; z++) {
      if (this.transactions[z].gt != null) {
        new_tx_fee = tx.returnFee();
        old_tx_fee = this.transactions[z].returnFee();
...
console.log(fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + ")");
      ...
      if (parseFloat(fees_available) >= parseFloat(fees_needed) && mempool_self.transactions.length > 0) {
