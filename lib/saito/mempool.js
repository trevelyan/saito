...
Mempool.prototype.addTransaction = function addTransaction(tx, relay_on_validate=1) {
  ...
  if (transaction_imported == 0) {
    ...
    this.app.storage.validateTransactionInputsForAddingToMempool(tx, function(app, tx) {
...
Mempool.prototype.startBundling = function startBundling() {
  ...
  this.bundling_timer = setInterval(function() {
    ...
    if (mempool_self.processing_blocks == 0 && mempool_self.creating_block == 0 && mempool_self.app.blockchain.currently_indexing == 0 && mempool_self.app.blockchain.reclaiming_funds == 0 && mempool_self.clearing_mempool == 0) {
...
console.log((new Date()) + ": " + fees_needed + " ---- " + fees_available + " (" + mempool_self.transactions.length + "/"+mempool_self.returnNormalTransactionsInMempool()+" -- "+mempool_self.app.wallet.returnBalance()+")");
      ...
      if (parseFloat(fees_available) >= parseFloat(fees_needed) && ( (mempool_self.returnNormalTransactionsInMempool() > 0 || mempool_self.app.wallet.returnBalance() < 1000000000) || mempool_self.app.blockchain.index.hash.length == 0)) {
...
Mempool.prototype.createBlock = function createBlock(prevblk) {
  ...
  // block to indexAndStore or when it fails 
