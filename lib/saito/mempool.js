...
function Mempool(app) {
  ...
  this.processing_speed           = 500; // 0.3 seconds
  ...
  this.bundling_speed             = 600; // 0.4 seconds
...
Mempool.prototype.debugHTML = function debugHTML() {

  var html  = '<hr width=100 />';
      html += '<code><pre>'+JSON.stringify(this.transactions, null, 4)+'</pre></code>';
...
Mempool.prototype.addTransaction = function addTransaction(tx, relay_on_validate=1) {
  ...
  if (tx.transaction.gt != null) {
	...
	if (this.transactions[z].transaction.gt.target == this.app.blockchain.returnLatestBlockHash()) {
	  ...
	  // this is not for the last block, so 
	  // we claim it is already imported in 
  ...
  if (transaction_imported == 0) {
	...
	// propagate transaction if this does not push us over
	// the limit of creating a block ourselves
	if ( this.bundling_fees_needed > tx.returnUsableFee() ) {
...
Mempool.prototype.startBundling = function startBundling() {
  ...
  this.bundling_timer = setInterval(function() {
	...
	latestBlk.block.id = 0; 
      ...
      this.bundling_fees_needed = feesneeded - fees_available;
...
Mempool.prototype.createBlock = function createBlock(prevblk) {
  ...
  for (var i = 0; i < this.transactions.length; i++) {
    var addtx = 1;
    if (this.transactions[i].transaction.gt != null) { 
console.log("CONSIDERING ADDING GT: ");
console.log(this.transactions[i].transaction.gt);
console.log("prev hash: "+prevblk.hash('hex'));
      if (this.transactions[i].transaction.gt.target != prevblk.hash('hex')) { 
console.log("NOT ACCEPTING THIS GOLDEN TICKET!");
        addtx = 0; 
      } 
    }
...
console.log(JSON.stringify(nb.block));

  // let the block finish itself
  nb.createBlock(prevblk);

console.log("I am the one creating this block: "+nb.hash('hex'));
