...
Storage.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_unwind_index, resetting_flag) {
    ...
    if (current_unwind_index == old_block_idxs.length-1) {
  ...
  windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_wind_index);
...
Storage.prototype.validateLongestChain = function validateLongestChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd) {
  ...
  if (old_block_hashes.length > 0) {
    this.unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, 0, 0);
  } else {
...
/*****
  //////////////////////////////////////////
  // prevent double-spending within block //
  //////////////////////////////////////////
  var block_is_valid = 1;
  var tmpgtfound = 0;
  var tmpftfound = 0;
  var spent_inputs = [];

  //
  // we go through the transactions and check that we do 
  // not include the same payment slip more than once. we
  // also check that there is only one golden-ticket and 
  // fee-ticket in any particular block.
  //
  for (var ti = 0; ti < newblock.transactions.length; ti++) {
    ...
    for (var ti2 = 0; ti2 < tx.transaction.from.length; ti2++) {
      ...
      if (tmpft == 1) {
        if (tmpftfound == 1) {
          console.log("Block invalid: multiple fee capture transactions in block");
          this.app.blockchain.addBlockToBlockchainFailure(newblock, tx, old_lc, lc_len);
          return 0;
        } else {
          tmpftfound = 1;
        }
