...
Storage.prototype.saveLongestChainStatus = function saveLongestChainStatus(block_hash, block_id, longest_chain_status) {
  ...
  if (longest_chain_status == 1) {
    ...
    sql = "UPDATE slips SET longest_chain = $longest_chain_status, spent = 1 WHERE block_id = $block_id AND block_hash = $block_hash";
    this.db.run(sql, {
      $block_id: block_id,
      $block_hash: block_hash,
      $longest_chain_status: longest_chain_status
...
Storage.prototype.spendSlip = function spendSlip(block_id, tx_id, slip_id, address) {
  ...
  sql = "UPDATE slips SET spent = 1 WHERE block_id = $block_id AND tx_id = $tx_id AND slip_id = $slip_id AND address = $address";
  params = {
    $block_id : block_id,
    $tx_id    : tx_id,
    $slip_id  : slip_id,
...
Storage.prototype.validateInputWithCallbackOnFailure = function validateInputWithCallbackOnFailure(tx, utxi, mycallback) {
  ...
  sql = "SELECT count(*) AS count FROM slips WHERE block_id = $block_id AND tx_id = $tx_id AND slip_id = $slip_id AND address = $address AND spent = 0";
...
Storage.prototype.validateInputsWithCallbackOnSuccess = function validateInputsWithCallbackOnSuccess(tx, utxiarray, mycallback) {
  ...
  for (via = 0; via < utxiarray.length; via++) {
    ...
    if (utxi.bid == 0 && utxi.tid == 0 && utxi.sid == 0 && utxi.gt == 1) { gtnum++; } else {
      if (sql == "") {
...
Storage.prototype.validateInputsWithCallbackOnFailure = function validateInputsWithCallbackOnFailure(tx, utxiarray, mycallback) {
