...
Storage.prototype.saveOptions = function saveOptions(reset_from_remote=0) {
  ...
  if (reset_from_remote == 1) {
    ...
    loadurl = '/client.options?x='+tmpdate;
...
console.log("PRODUCING OPTIONS FILE: ");
console.log(tmpjson);
...
Storage.prototype.saveLongestChainStatus = function saveLongestChainStatus(block_hash, block_id, longest_chain_status) {
  ...
  // blocks
  sql = "UPDATE blocks SET longest_chain = $longest_chain_status WHERE block_id = $block_id AND hash = $block_hash";
  this.db.run(sql, {
    $block_id: block_id,
  ...
  // transactions
  sql = "UPDATE txs SET longest_chain = $longest_chain_status WHERE block_id = $block_id AND block_hash = $block_hash";
  this.db.run(sql, {
    $block_id: block_id,
    $block_hash: block_hash,
    $longest_chain_status: longest_chain_status
  ...
  // "unspend" our spent transaction inputs
  if (longest_chain_status == 0) {

    sql = "UPDATE slips SET longest_chain = $longest_chain_status, spent = 0 WHERE block_id = $block_id AND block_hash = $block_hash";
    this.db.run(sql, {
      $block_id: block_id,
      $block_hash: block_hash,
      $longest_chain_status: longest_chain_status
  ...
  if (longest_chain_status == 1) {

    sql = "UPDATE slips SET longest_chain = $longest_chain_status, spent = $spent_block_id WHERE block_id = $block_id AND block_hash = $block_hash";
    this.db.run(sql, {
...
Storage.prototype.saveBlock = function saveBlock(blk, lc=0) {
  ...
  sql = "INSERT OR IGNORE INTO blocks (block_id, block, hash, conf, longest_chain) VALUES ($block_id, $block, $hash, 0, $lc)";
  this.db.run(sql, {
...
Storage.prototype.saveTransaction = function saveTransaction(block_id, tx, block_hash, lc=0) {
  ...
  if (this.app.BROWSER == 1) { return; }
  sql = "INSERT OR IGNORE INTO txs (block_id, block_hash, tx_id, tx, longest_chain) VALUES ($block_id, $block_hash, $tx_id, $tx, $lc)";
  this.db.run(sql, {
    $block_id: block_id,
    $block_hash: block_hash,
  ...
  for (bba = 0; bba < tx.transaction.from.length; bba++) {
    this.spendSlip(tx.transaction.from[bba].bid, tx.transaction.from[bba].tid, tx.transaction.from[bba].sid, tx.transaction.from[bba].add);
  }
...
Storage.prototype.spendSlip = function spendSlip(block_id, tx_id, slip_id, address) {

  if (this.app.BROWSER == 1) { return; }

  sql = "UPDATE slips SET spent = $spent_block_id WHERE block_id = $block_id AND tx_id = $tx_id AND slip_id = $slip_id AND address = $address";
  params = {
    $spent_block_id : block_id,
    $block_id : block_id,
    $tx_id    : tx_id,
...
Storage.prototype.validateInputsWithCallbackOnSuccess = function validateInputsWithCallbackOnSuccess(tx, mycallback) {
  ...
  for (via = 0; via < utxiarray.length; via++) {

    utxi  = utxiarray[via];

    if (utxi.amt == 0 && utxi.bid == 0 && utxi.tid == 0 && utxi.sid == 0 && utxi.gt == 1) { gtnum++; } else {
      if (sql == "") {
        sql    = "SELECT count(*) AS count FROM slips WHERE (block_id = "+parseInt(utxi.bid)+" AND tx_id = "+parseInt(utxi.tid)+" AND slip_id = "+parseInt(utxi.sid)+" AND spent = 0 AND address = $address) ";
        params = { $address : utxi.add };
      } else {
        sql    = sql + " OR (block_id = "+parseInt(utxi.bid)+" AND tx_id = "+parseInt(utxi.tid)+" AND slip_id = "+parseInt(utxi.sid)+" AND spent = 0 AND address = $address) ";
        params = { $address : utxi.add };
      }
    }
  }
...
Storage.prototype.validateInputsWithCallbackOnFailure = function validateInputsWithCallbackOnFailure(blk, tx, spent_block_id, unacceptable_block_hashes_sql_list, old_lc, lchain_len, mycallback) {
  ...
  for (via = 0; via < utxiarray.length; via++) {
    ...
    if (sql == "") {
      sql    = "SELECT count(*) AS count FROM slips WHERE (block_id = "+parseInt(utxi.bid)+" AND tx_id = "+parseInt(utxi.tid)+" AND slip_id = "+parseInt(utxi.sid)+" AND (       (    (spent = 0 OR spent <= "+parseInt(spent_block_id)+") OR (spent > "+parseInt(spent_block_id)+" AND block_hash NOT IN ("+unacceptable_block_hashes_sql_list+")      )     )     ) AND address = $address) ";
      params = { $address : utxi.add };
    } else {
      sql    = sql + " OR (block_id = "+parseInt(utxi.bid)+" AND tx_id = "+parseInt(utxi.tid)+" AND slip_id = "+parseInt(utxi.sid)+" AND  (     (    (spent = 0 OR spent <= "+parseInt(spent_block_id)+") OR (spent > "+parseInt(spent_block_id)+" AND block_hash NOT IN ("+unacceptable_block_hashes_sql_list+")     )      )     )   AND address = $address) ";
      params = { $address : utxi.add };
    }
  }
...
Storage.prototype.validateTransactionInputs = function validateTransactionInputs(myblk, chain_hashes, old_lc, lchain_len) {
  ...
  chain_hash_sql = "";
  for (schc = 0; schc < chain_hashes.length; schc++) {
    if (chain_hash_sql.length > 1) { chain_hash_sql += ","; }
    chain_hash_sql += '"'+chain_hashes[schc]+'" ';
  }

  for (mbi = 0; mbi < myblk.transactions.length; mbi++) {
    ...
    this.validateInputsWithCallbackOnFailure(myblk, myblk.transactions[mbi], myblk.returnId(), chain_hash_sql, old_lc, lchain_len, function(blk, tx, old_lc) {
      ...
      formerchain = storage_self.app.blockchain.returnLongestChainIndex(lchain_len);
