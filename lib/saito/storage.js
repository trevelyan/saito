...
Storage.prototype.onChainReorganization = function onChainReorganization(block_id, block_hash, lc) {
  ...
  this.openBlockByHash(block_hash, function(storage_self, blk) {
    for (var x = 0; x < blk.transactions.length; x++) {
      ...
      if (lc == 0) {
	  storage_self.unspendTransactionInputs(blk, blk.transactions[x]);
      }

      if (lc == 1) {

	// update spent slips
        if (storage_self.validateTransactionInputs(blk, blk.transactions[x], 1) == 1) {
	  storage_self.spendTransactionInputs(blk, blk.transactions[x]);

	// if an input is invalid, unspend txs done to date
	} else {

	  for (var xx = x-1; xx >= 0; xx--) {
	    // undo block to this point
	    storage_self.unspendTransactionInputs(blk, blk.transactions[x]);
	  }
...
/////////////////////////////////
Storage.prototype.validateTransactionInputs = function validateTransactionInputs(blk, tx) {
    ...
    var utxi  = utxiarray[via];
     ...
     if (utxi.amt == 0) {
       ...
       if (utxi.amt == 0 && utxi.bid == 0 && utxi.tid == 0 && utxi.sid == 0 && (utxi.gt == 1 || utxi.ft == 1)) { gtnum++; } else {
         ...
         if (shashmap.validate_slip(slip_map_index, blk.block.id) == 1) { map_found++; }
       }
     }
   }

  if (gtnum == utxiarray.length) { return 1; }
  if (gtnum+map_found >= utxiarray.length) { return 1; }

console.log("\n\nFAILED VALIDATION: " + gtnu + " - " + map_found);
console.log(JSON.stringify(utxi);
