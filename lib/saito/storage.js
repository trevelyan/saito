...
console.log("STORAGE SENDING: "+start_bid + " ---> " + peer.sync_latest_bid);

  peer.sync_timer = setInterval(function() {

console.log("timer tick: " + peer.message_queue.length + " - " + storage_self.send_blocks_queue_limit + " - " + peer.sync_sending_chunk);   
...
console.log(params);
    ...
    storage_self.db.get(sql, params, function (err, rowz) {
      if (rowz != null) {
        ...
        if (count > 0) {
	  if (peer.message_queue.length < storage_self.send_blocks_queue_limit) {
console.log("SENDING BLOCKCHAIN CHUNK");
...
console.log("ABOUT TO SEND: "+rows[r].block_id);
