...
Storage.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_unwind_index) {
...
Storage.prototype.windChain = function windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_wind_index) {
...
Storage.prototype.validateLongestChain = function validateLongestChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd) {
  ...
  if (old_block_hashes.length > 0) {
    ...
    this.unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, 0);
  } else {

    this.windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, 0);
  ...
  if (blocks_to_validate_sequentially == 1) {
	...
	var blocks_validated   = 0;
        ...
        for (var sqlr = 0; sqlr < rows.length; sqlr++) {
          ...
          try {
	    storage_self.openBlockByFilename(files[current_block], function(storage_self, tmpblk) {
