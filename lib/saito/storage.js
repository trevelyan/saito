...
console.log(" tx add mempool validate ... 1");
...
console.log(" tx add mempool validate ... 1.5: " + gtnum + " -- " + utxiarray.length);

  if (gtnum == utxiarray.length) { mycallback(this.app, tx); return; }
console.log(" tx add mempool validate ... 1.7: ");
console.log(sql);
console.log("CURRENTLY INDEXING? " + this.app.blockchain.currently_indexing);
console.log(" tx add mempool validate ... 2: ");
  this.queryBlockchain(sql, params, function (err, row) {
console.log(" tx add mempool validate ... 22");
    if (row != null) {
console.log(" tx add mempool validate ... 3");
console.log("ROW COUNTL ");
console.log(row);
      if (row.count > 0) {
console.log(" tx add mempool validate ... 4");
        if ((row.count+gtnum) == utxiarray.length) {
console.log(" tx add mempool validate ... 5");
...
console.log("creating block with this json");
...
Storage.prototype.sendBlockchainChunk = function sendBlockchainChunk(peer) {
  ...
  // send 10 per chunk
  var sql    = "SELECT * FROM blocks JOIN blocks_json WHERE blocks.block_id >= $block_id AND blocks.id >= $db_id AND blocks.block_json_id = blocks_json.id ORDER BY block_id, id ASC LIMIT 10";
  var params = { $block_id : peer.sync_sending_bid , $db_id : peer.sync_sending_db_bid };
