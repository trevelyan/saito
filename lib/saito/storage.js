...
console.log("\n\nSAVING CLIENT: "+tmpjson.lastblock+"\n\n");
...
Storage.prototype.saveSlip = function saveSlip(block_id, tx_id, slip_id, slip_json, slip) {
  ...
  sql = "INSERT OR IGNORE INTO slips (block_id, tx_id, slip_id, slip_json, golden_ticket, address, amount, spent) VALUES ($block_id, $tx_id, $slip_id, $slip_json, $address, $golden_ticket, $amount, $spent)";
  this.db.run(sql, {
    $block_id: block_id,
    $tx_id: tx_id,
    $slip_id: slip_id,
...
console.log("SAVING BROWSER CACHE");
...
Storage.prototype.indexRecentBlocks = function indexRecentBlocks(mylimit=10) {
  ...
  this.db.all(sql, {
    ...
    for (sqlr = 0; sqlr < rows.length; sqlr++) {
      var blk = new trust.block(storage_self.app, rows[sqlr].block);
      ...
      storage_self.app.blockchain.indexAndStore(new trust.block(storage_self.app, rows[sqlr].block), "force");
    }
  });
