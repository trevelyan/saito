...
//
Storage.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_unwind_index, resetting_flag) {
  ...
  if (old_block_hashes.length > 0) {
    ...
    storage_self.openBlockByHash(old_block_hashes[current_unwind_index], function(storage_self, blk) {
      ...
      if (current_unwind_index == 0) {
        windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, 0, resetting_flag);
      } else {
        unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_unwind_index-1, resetting_flag);
      }
...
Storage.prototype.windChain = function windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_wind_index, resetting_flag) {
  ...
  if (this_block_hash == newblock.returnHash()) {
	...
	// into addBlockToBlockchainFailure.
        //
	storage_self.windChain(newblock, pos, shared_ancestor_index_pos, old_block_idxs, old_block_hashes, null, null, i_am_the_longest_chain, forceAdd, 0, 1);
	...
	// swap our hashes to wind/unwind.
        ...
        var chain_to_unwind_idxs   = new_block_hashes.splice(current_wind_index);

        // we need to unwind the stuff we've already WINDED and rewind the old chain.
	storage_self.unwindChain(newblock, pos, shared_ancestor_index_pos, old_block_idxs, old_block_hashes, chain_to_unwind_hashes, chain_to_unwind_idxs, i_am_the_longest_chain, forceAdd, chain_to_unwind_hashes.length, 1);
  ...
  // same thing as above, essentially.
	...
	// if this is the last block, we push into either Part Two or Failed
	  ...
	  if (resetting_flag == 0) {
          ...
          windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, current_wind_index+1, resetting_flag);
        }
...
Storage.prototype.validateLongestChain = function validateLongestChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd) {
  ...
  if (old_block_hashes.length > 0) {
    this.unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, old_block_hashes.length-1, 0);
  } else {
    this.windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs, i_am_the_longest_chain, forceAdd, 0, 0);
  }
