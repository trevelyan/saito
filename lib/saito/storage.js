...
Storage.prototype.saveBlock = function saveBlock(blk, lc = 0) {
  ...
  for (var b = 0; b < blk.transactions.length; b++) {
    ...
    for (var bb = 0; bb < blk.transactions[b].transaction.from.length; bb++) {
	  ...
	  // this is a failure to validate a slip. we will permit it if 
	  // we do not have a full genesis period worth of blocks
	  if (blk.transactions[b].transaction.from[bb].bid < storage_self.app.blockchain.blk_limit) {

	    // this is OK. we may be on a poisoned chain, but we will
	    // not know until we have a full genesis period worth of 
	    // transactions.
