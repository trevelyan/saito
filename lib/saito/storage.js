...
Storage.prototype.saveLongestChainStatus = function saveLongestChainStatus(block_hash, block_id, longest_chain_status) {
  ...
  console.log( "2 - UPDATE slips SET spent = "+bid+" WHERE block_id = "+bid+" AND tx_id = "+tid+" AND slip_id = "+sid+" AND block_hash = \""+block_hash+"\" AND longest_chain = "+tmp_lcs);
      		storage_self.db.run(sql, params, function(err) {
          });
        }
      }
...
Storage.prototype.spendSlip = function spendSlip(block_id, tx_id, slip_id, address, block_hash) {
  ...
  console.log( "1 --- UPDATE slips SET spent = "+block_id+" WHERE block_id = "+block_id+" AND tx_id = "+tx_id+" AND slip_id = "+slip_id+" AND longest_chain = 1 AND address = \""+address+"\"");
