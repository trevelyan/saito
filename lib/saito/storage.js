...
Storage.prototype.saveOptions = function saveOptions(reset=0) {
  ...
  if (reset == 1) {
	...
	if (storage_self.app.BROWSERIFY == 1) { alert("Account reset: please refresh your browser"); }
    ...
    if (this.app.options.friends == null) {
      this.app.options.friends  = this.app.friends.returnFriends();
    }
...
Storage.prototype.ifNewMessage = function ifNewMessage(tx, callback) {

  if (this.app.BROWSERIFY == 1) {

    // browsers store messages in browser cache
    for (bvc = 0; bvc < this.app.options.messages.length; bvc++) {
      if (this.app.options.messages[bvc] == JSON.stringify(tx.transaction)) { return; }
    }
    ...
    sql = "SELECT count(*) AS count FROM txs WHERE text = $txtext";
    params = { $txtext : JSON.stringify(tx.transaction) }
console.log("A: "+sql);
    this.queryBlockchain(sql, params, function (err, row) {
console.log("B: "+row);
      if (row != null) {
console.lg("C: "+row.count);
        if (row.count == 0) {
console.lg("D: "+callback);
	  callback(tx);
        }
      }
    });
...
Storage.prototype.indexRecentBlocks = function indexRecentBlocks(mylimit=10) {
  ...
  this.db.all(sql, {
    ...
    for (sqlr = 0; sqlr < rows.length; sqlr++) {
      var blk = new saito.block(storage_self.app, rows[sqlr].block, rows[sqlr].conf); 
