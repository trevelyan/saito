  ...
  storage_self.queryBlockchainArray(sql, params, function (err, rows) {
    if (rows.length > 0) {
      for (var x = 0; x < rows.length; x++) {
        var block_filename = __dirname + "/../data/blocks/" + rows[x].block_id + "-" + rows[x].id + ".blk.zip";
  ...
  storage_self.db.run(sql2, {
    ...
    if (this.lastID != undefined) {
      ...
      if (blk.tmp_filename != "") {
        ...
        var source_file = "/Users/admin/Programming/saito/lib/data/tmp/" + blk.tmp_filename;
        blk.pmt_filename = blk.block.id + "-" + this.lastID + ".blk.zip";
        var dest_file   = "/Users/admin/Programming/saito/lib/data/blocks/" + blk.pmt_filename;
      ...
      /////////////////////
        ...
        blk.pmt_filename    = blk.block.id + "-" + this.lastID + ".blk.zip";
        var block_filename2 = __dirname + "/../data/blocks/" + blk.pmt_filename;
        ...
        if ( ! fs.existsSync(block_filename2)) {
          ...
          var block_filename4 = __dirname + "/../data/blocks/" + blk.block.id + "-sa.segadd";
	  blk.compressSegAdd();
...
Storage.prototype.openBlockByHash = function openBlockByHash(block_hash, mycallback) {
  ...
  this.db.all(sql, params, function (err, rows) {
    if (rows.length > 0) {
      ...
      var block_filename2 = __dirname + "/../data/blocks/" + rows[0].block_id + "-" + rows[0].id + ".blk.zip";
      try {
...
Storage.prototype.openBlockByFilename = function openBlockByFilename(filename, mycallback) {
  ...
  var block_filename2 = __dirname + "/../data/blocks/" + filename + ".zip";
  var blkjson = "";
...
Storage.prototype.openBlockByZipFilename = function openBlockByZipFilename(zip_filename, mycallback) {
  ...
  var block_filename2 = __dirname + "/../data/blocks/" + zip_filename;
  var blkjson = "";
...
Storage.prototype.openBlockByTmpZipFilename = function openBlockByTmpZipFilename(zip_filename, orig_filename, mycallback) {
  ...
  var block_filename2 = "/Users/admin/Programming/saito/lib/data/tmp/" + zip_filename;
  var blkjson = "";
