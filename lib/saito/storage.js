...
Storage.prototype.saveLongestChainStatus = function saveLongestChainStatus(block_hash, block_id, longest_chain_status) {
  ...
  var sql = "UPDATE slips SET longest_chain = $longest_chain_status WHERE block_id = $block_id AND block_hash = $block_hash";
  //console.log("UPDATE slips SET longest_chain = "+longest_chain_status+" WHERE block_id = "+block_id+" AND block_hash = "+block_hash);
  this.db.run(sql, {
    $block_id: block_id,
    $block_hash: block_hash,
    $longest_chain_status: longest_chain_status
  ...
  this.queryBlockchainArray(sql, params, function(err, rows) {
	...
	if (tmp_lcs == 0) {
	  ...
	  var sql = "UPDATE slips SET spent = 0 WHERE block_hash = $block_hash AND block_id = $block_id AND tx_id = $tx_id AND slip_id = $slip_id";
          //console.log("UPDATE slips SET spent = 0 WHERE block_hash = \""+bhash+"\" block_id = "+bid+" AND tx_id = "+tid+" AND slip_id = "+sid);
      	  var params =  {
	    $block_hash: bhash,
	    $block_id: bid,
	    $tx_id: tid,
	  ...
	  var sql = "UPDATE slips SET spent = $block_spent WHERE block_hash = $block_hash AND block_id = $block_id AND tx_id = $tx_id AND slip_id = $slip_id";
  ...
  for (var via = 0; via < utxiarray.length; via++) {
    ...
    if (utxi.amt == 0 && utxi.bid == 0 && utxi.tid == 0 && utxi.sid == 0 && (utxi.gt == 1 || utxi.ft == 1)) { gtnum++; } else {
      if (sql == "") {
        ...
        sql_error = "SELECT * FROM slips WHERE (longest_chain = 1 AND block_id = "+parseInt(utxi.bid)+" AND tx_id = "+parseInt(utxi.tid)+" AND slip_id = "+parseInt(utxi.sid)+" AND (spent = 0 OR spent = "+parseInt(blk.returnId())+") AND address = \""+utxi.add+"\") ";
      } else {
        ...
        sql_error = sql_error + " OR (longest_chain = 1 AND block_id = "+parseInt(utxi.bid)+" AND tx_id = "+parseInt(utxi.tid)+" AND slip_id = "+parseInt(utxi.sid)+" AND (spent = 0 OR spent = "+parseInt(blk.returnId())+") AND address = \""+utxi.add+"\") ";
      }
    }
  }
...
console.log(sql_error);
          storage_self.queryBlockchainArray(sql_error, params, function (err2, rows2) {
...
console.log("failure validation 1");
console.log(sql);
        }
      } else {
...
Storage.prototype.validateTransactionInputs = function validateTransactionInputs(myblk, chain_hashes, old_lc, lchain_len, blocks_to_validate_sequentially) {
  ...
  for (var ti = 0; ti < myblk.transactions.length; ti++) {
	...
	if (tmpftfound == 1) { 
      ...
      if (tmpgt == 1) {
	if (tmpgtfound == 1) { 
	  console.log("Block invalid: multiple golden ticket transactions in block");
          this.app.blockchain.failedBlockTransactionValidation(myblk, tx, old_lc, lchain_len);
	  return 0;
	} else {
	  tmpgtfound = 1;
      ...
      var as_indexer = "a"+tmpbid+"-"+tmptid+"-"+tmpsid+"-"+tmpgt+"-"+tmpft;
      if (spent_inputs[as_indexer] == 1) {
	console.log("Block invalid: multiple transactions spend same input: "+tmpbid+"/"+tmptid+"/"+tmpsid+"/"+tmpgt+"/"+tmpft);
        this.app.blockchain.failedBlockTransactionValidation(myblk, tx, old_lc, lchain_len);
	return 0;
      }
  ...
  if (blocks_to_validate_sequentially == 1) {
    ...
    var sql = "SELECT * FROM txs WHERE block_id >= $min_block_id AND block_id <= $max_block_id AND longest_chain = 1 ORDER BY tx_id ASC";
    var params = {
