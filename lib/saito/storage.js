...
console.log("\n\nTESTING THE CREATING DATABASE TABLES\n\n");
...
Storage.prototype.saveBlock = function saveBlock(blk, lc = 0) {
  ...
  this.db.run(sql, {
   ...
   console.log("CHECKING INSERT ID: "+this.lastID);
...
Storage.prototype.saveTransaction = function saveTransaction(block_id, tx, block_hash, lc=0) {
  ...
  var sql = "INSERT OR IGNORE INTO txs_json (tx_json) VALUES ($tx)";
  this.db.run(sql, {
...
//  console.log("INSERT OR IGNORE INTO slips (block_id, tx_id, slip_id, slip_json, golden_ticket, fee_ticket, address, amount, block_hash, longest_chain, spent) VALUES ("+block_id+", "+tx_id+", "+slip_id+", '"+slip_json+"', '"+slip.gt+"', "+slip.ft+", '"+slip.add+"', "+slip.amt+", '"+bhash+"', "+lc+", 0)");
  var params = {
    $block_id: block_id,
    $tx_id: tx_id,
    $slip_id: slip_id,
