...
Storage.prototype.unwindChain = function unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, current_unwind_index, resetting_flag) {
  ...
  if (old_block_hashes.length > 0) {
    ...
    storage_self.openBlockByHash(old_block_hashes[current_unwind_index], function(storage_self, blk) {
      ...
      storage_self.app.storage.onChainReorganization(blk.block.id, blk.block.returnHash(), 0);
      storage_self.app.wallet.onChainReorganization(blk.block.id, blk.block.returnHash(), 0);
      storage_self.app.modules.onChainReorganization(blk.block.id, blk.block.returnHash(), 0);

      storage_self.unspendBlockSlips(blk);
      ...
      if (current_unwind_index == 0) {
        windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, 0, resetting_flag);
      } else {
        unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, current_unwind_index-1, resetting_flag);
      }
...
Storage.prototype.unspendBlockInputs = function unspendBlockInputs(blk) {
  ...
  for (var x = 0; x < blk.transactions.length; x++) {
    for (var y = 0; y < blk.transactions[x].transaction.from.length; y++) {
      ...
      //////////////////////////
      var slip_map_index = this.returnHashmapIndex(bid, tid, sid, add, amt, block_hash);
      shashmap.insert_slip(slip_map_index, -1);
    }
  }
...
Storage.prototype.windChain = function windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, current_wind_index, resetting_flag) {
  ...
  if (this_block_hash == newblock.returnHash()) {
    ...
    if (this.validateBlockInputs(newblock) == 1) {
      ...
      // handle onChainReorganization calls
      storage_self.app.storage.onChainReorganization(blk.block.id, blk.block.returnHash(), 1);
      storage_self.app.wallet.onChainReorganization(blk.block.id, blk.block.returnHash(), 1);
      storage_self.app.modules.onChainReorganization(blk.block.id, blk.block.returnHash(), 1);
  ...
  // same thing as above, essentially.
    ...
    storage_self.openBlockByHash(old_block_hashes[current_wind_index], function(storage_self, blk) {
	...
	// if this is the last block, we push into either Part Two or Failed
	  ...
	  if (resetting_flag == 0) {
            ...
            this.app.blockchain.addBlockToBlockchainFailure(newblock, pos, i_am_the_longest_chain, forceAdd, old_block_idxs[current_unwind_index]);
	  }
        } else {
          windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, current_wind_index+1, resetting_flag);
        }
...
// HACK
        ...
        console.log("shit, we need to re-roll our old chain....");
...
console.log("WE SHOULD NEVER HAVE HIT THIS, RIGHT?");
process.exit();
      if (current_wind_index == new_block_idxs.length-1) {
        this.app.blockchain.addBlockToBlockchainPartTwo(newblock, pos, i_am_the_longest_chain, forceAdd);
      } else {
        windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_idxs, old_block_ids, i_am_the_longest_chain, forceAdd, current_wind_index+1, resetting_flag);
      }
...
Storage.prototype.validateLongestChain = function validateLongestChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd) {
  ...
  if (old_block_hashes.length > 0) {
    this.unwindChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids_idxs, i_am_the_longest_chain, forceAdd, old_block_hashes.length-1, 0);
  } else {
    this.windChain(newblock, pos, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, new_block_ids, old_block_idxs, old_block_hashes, old_block_ids, i_am_the_longest_chain, forceAdd, 0, 0);
  }
...
Storage.prototype.openBlockByHash = function openBlockByHash(block_hash, mycallback) {
  ...
  this.db.all(sql, params, function (err, rows) {
    if (rows.length > 0) {
      ...
      openBlockByFilename(block_filename, mycallback);
