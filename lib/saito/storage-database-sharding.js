...
function Storage(app) {
  ...
  this.defragment_limit           = 2; // every 100 blocks, defragment the DB
  ...
  this.bhash_to_validate          = 0;
  this.block_to_validate          = null;
  this.txs_to_validate            = 0;
  ...
  this.txs_validated              = 0;
  this.children_validated         = 0;

  ///////////////////////
  // scaling variables //
  ///////////////////////
  this.save_blocks_in_db          = 0;  // 1 to save in db, 0 to write to disk
  this.memory_only_db             = 0;  // 1 for memory-only database

  //////////////
  // clusters //
  //////////////
  this.enable_clustering  = 1;	// 1 means split the slips database into 
				// sections with one per CPU core. This 
				// should hypothetically speed up writing
				// to the database and recalculating indexes, etc.
  this.number_of_clusters = 3;  // if we manually specify;
  this.child_forks        = [];
  //
  // HACK
  //
  // this is a temporary var used in ChildFork function
  // when testing with teh spammer module. It allows us
  // to avoid calling random numbers when saving.
  this.fork_counter       = 0;
...
// Initialize //
////////////////
Storage.prototype.pre_initialize = function pre_initialize() {
  ...
  // CLUSTERING //
  ////////////////
  // to disable clustering, just uncomment
  // return;
  ...
  for (var f = 0; f < number_of_forks; f++) {
    ...
    //console.log("START cluster: "+f);

    var forked = fork('child.js');
    forked.on('message', (msg) => {

      if (msg.slipSaved != undefined) {
        storage_self.slips_saved += msg.slipSaved;
        if (storage_self.slips_saved >= storage_self.slips_to_save) {
          storage_self.saving_slips = 0;
        }
      }

      if (msg.slipSpent != undefined) {
        storage_self.slips_spent += msg.slipSpent;
        if (storage_self.slips_spent >= storage_self.slips_to_spend) {
          storage_self.spending_slips = 0;
        }
      }

      if (msg.slipValidated != undefined) {
        storage_self.slips_validated += msg.slips_validated;
        if (storage_self.bhash_to_validate == msg.block_hash) {
          if (storage_self.slips_validated >= storage_self.slips_to_validate) {
            storage_self.app.blockchain.successfulBlockTransactionValidation(storage_self.app, null, storage_self.block_to_validate, msg.forceAdd, storage_self.slips_validated); 
            storage_self.validating_slips = 0;
            storage_self.bhash_to_validate = "";
          }
          return;
        }
	storage_self.children_validated++;
        if (storage_self.children_validated >= storage_self.child_forks.length) {
	  storage_self.app.blockchain.failedBlockTransactionValidation(storage_self.block_to_validate, null, msg.old_lc, msg.lc_len);
          storage_self.validating_slips = 0;
          storage_self.bhash_to_validate = "";
	}
      }
...
Storage.prototype.initialize = function initialize() {
  ...
  if (this.app.BROWSER == 0) {
    ...
    // increase page size to 16384 (default is 4096 -- keep to power of two) 
    this.execDatabase("PRAGMA page_size = 8192", {}, function (){});

    // increase cache size to 10000 bytes (default is 1024) 
    //this.execDatabase("PRAGMA cache_size = 20480", {}, function (){});
    //this.execDatabase("PRAGMA cache_size = 102400", {}, function (){});
...
// 	   	storage_self.app.storage.execDatabase("CREATE INDEX slips_json_idx ON slips_json (block_id)", {}, function() {});
  ...
  this.execDatabase("\
	...
	function() {
 	   	...
 	   	storage_self.app.storage.execDatabase("CREATE INDEX slips_idx4 ON slips (longest_chain, block_id, tx_id, slip_id, spent, address)", {}, function() {});
	}
  );
...
// Longest Chain //
///////////////////
Storage.prototype.saveLongestChainStatus = function saveLongestChainStatus(block_hash, block_id, lc, am_i_child=0) {
  ...
  if (am_i_child == 1) {
    ...
    // CLUSTERING //
    ////////////////
    console.log("We need to update the longest chain function for child forks...");
    process.exit();
...
Storage.prototype.saveBlock = function saveBlock(blk, lc = 0, am_i_child = 0, child_number = 0, max_children = 0) {
  ...
  // CLUSTERING //
  ////////////////
  if (this.child_forks.length > 0) {

    for (var bi = 0; bi < blk.transactions.length; bi++) {
      this.slips_to_save  += blk.transactions[bi].transaction.to.length;
      this.slips_to_spend += blk.transactions[bi].transaction.from.length;
    }
    this.slips_to_validate = this.slips_to_spend;
 
    var blk_json = JSON.stringify(blk.block);

    for (var cf = 0; cf < this.child_forks.length; cf++) {
      this.child_forks[cf].send({ saveBlock: blk_json , lc : lc , child_number : cf , max_children : this.child_forks.length });
    }

    //console.log("PARENT: " + this.slips_to_save + " - " + this.slips_to_spend);
  ...
  storage_self.db.run(sqlbegin, {}, function() {
    ...
    if (am_i_child == 0) {
      ...
      // SAVE BLOCK TO DB //
      //////////////////////
      if (this.save_blocks_in_db == 1) {

        var sql = "INSERT INTO blocks_json (block_json, block_id) VALUES ($block, $block_id)";
        storage_self.db.run(sql, {
          $block: JSON.stringify(blk.block), 
          $block_id : blk.block.id
        }, function(err) {

          var sql2 = "INSERT INTO blocks (block_id, reindexed, block_json_id, hash, conf, longest_chain) VALUES ($block_id, 1, $block_json_id, $hash, 0, $lc)";
          storage_self.db.run(sql2, {
            ...
            // save BLOCK TO DISK
            var block_filename = __dirname + "/../data/blocks/" + blk.block.id + "-" + this.lastID + ".blk";
            fs.writeFileSync(block_filename, JSON.stringify(blk.block), function(err) {
              if(err) {
                return console.log(err);
              }
            });
      ...
      ////////////////////////
        ...
        storage_self.db.run(sql2, {
          ...
          var block_filename = "data/blocks/" + blk.block.id + "-" + this.lastID + ".blk";
          fs.writeFileSync(block_filename, JSON.stringify(blk.block), function(err) {
            if(err) {
              return console.log(err);
            }
    ...
    // am_i_child == 0
      ...
      for (var b = 0; b < blk.transactions.length; b++) {
        for (var bb = 0; bb < blk.transactions[b].transaction.to.length; bb++) {
          var which_child = storage_self.returnChildForkForSlip(blk.transactions[b].transaction.to[bb]);
          if (which_child == child_number) {
    	    storage_self.slips_to_save++;
          }
        }
      }
      this.fork_counter = 0;
      for (var b = 0; b < blk.transactions.length; b++) {
        for (var bb = 0; bb < blk.transactions[b].transaction.from.length; bb++) {
          var which_child = storage_self.returnChildForkForSlip(blk.transactions[b].transaction.from[bb]);
          if (which_child == child_number) {
  	    storage_self.slips_to_spend++;
          }
        }
      }

//console.log("CHILD COUNT: " + storage_self.slips_to_save + " -- " + storage_self.slips_to_spend);

    }

    this.fork_counter = 0;
    for (var b = 0; b < blk.transactions.length; b++) {
      storage_self.saveTransaction(blk.block.id, blk.transactions[b], blk.hash(), lc, am_i_child, child_number, max_children);
    }

  }); // everything is wrapped in the begin tx

}
Storage.prototype.saveTransaction = function saveTransaction(block_id, tx, block_hash, lc=0, am_i_child = 0, child_number = 0, max_children = 0) {
...
/*
  *
  * TRANSACTION JSON -- NOW WE DO NOT STORE TX DATA AT ALL
  *
  //var sql = "INSERT OR IGNORE INTO txs_json (tx_json, block_id) VALUES ($tx, $block_id)";
  var sql = "INSERT INTO txs_json (tx_json, block_id) VALUES ($tx, $block_id)";
  this.db.run(sql, {
    ...
    var sql2 = "INSERT INTO txs (block_id, block_hash, tx_id, tx_json_id, longest_chain) VALUES ($block_id, $block_hash, $tx_id, $tx_json_id, $lc)";
  ...
  */
  //
  //
  // if we aren't saving txs to db, do this
  storage_self.saving_txs = 0;
  /*
  *
  *
  */
  ...
  if (am_i_child == 1) {
    for (var bb = 0; bb < tx.transaction.to.length; bb++) {
      var which_child = this.returnChildForkForSlip(tx.transaction.to[bb]);
      if (which_child == child_number) {
        this.saveSlip(block_id, tx.transaction.id, bb, JSON.stringify(tx.transaction.to[bb]), tx.transaction.to[bb], block_hash, lc, 1);
      }
    }
    for (var bba = 0; bba < tx.transaction.from.length; bba++) {
      var which_child = this.returnChildForkForSlip(tx.transaction.to[bb]);
      if (which_child == child_number) {
        this.spendSlip(block_id, tx.transaction.from[bba].bid, tx.transaction.from[bba].tid, tx.transaction.from[bba].sid, tx.transaction.from[bba].add, tx.transaction.from[bba].bhash, 1);
      }
    }
  } else {
...
Storage.prototype.returnChildForkForSlip = function returnChildForkForSlip(s) {


  // number of clusters children
  this.fork_counter++;
  if (this.fork_counter >= this.number_of_clusters) { this.fork_counter = 0; }
  return this.fork_counter; 

//console.log("VAR: "+s.bhash + " -- " + s.bhash.length);
//console.log(JSON.stringify(s));
//  if (s.bhash.length < 2) { return 0; }
//  else {
//    var x = s.bhash.charCodeAt(1);
//    var y = this.child_forks.length;
//console.log("INSERTING INTO: " + x%y);
//    return x%y;
//  }

}
Storage.prototype.saveSlip = function saveSlip(block_id, tx_id, slip_id, slip_json, slip, bhash, lc, am_i_child=0) {

  if (this.app.BROWSER == 1) { return; }

  var storage_self = this;

  // we don't need to save slips worth nothing, as they
  ...
  if (slip.amt == 0) {
    ...
    if (am_i_child == 1) {
      ...
      if (storage_self.slips_saved == storage_self.slips_to_save) {
        process.send({ slipSaved : storage_self.slips_saved });
        storage_self.saving_slips = 0;
        if (storage_self.spending_slips == 0) { storage_self.execDatabase("COMMIT", {}, function(err) {}); }
      }
    } else {
      storage_self.slips_saved++;
      if (storage_self.slips_saved == storage_self.slips_to_save) {
 ...
 *
  ...
  var sql = "INSERT INTO slips_json (slip_json, block_id) VALUES ($slip, $block_id)";
  this.db.run(sql, {
    $slip: slip_json,
    $block_id: block_id
  }, function(err) {
 */
  this.lastID = 10;
    ...
    var sql2 = "INSERT INTO slips (block_id, tx_id, slip_id, slip_json_id, golden_ticket, fee_ticket, address, amount, block_hash, longest_chain, spent) VALUES ($block_id, $tx_id, $slip_id, $slip_json_id, $golden_ticket, $fee_ticket, $address, $amount, $bhash, $lc, $spent)";

//    console.log(this.dbname);
//    console.log("INSERT INTO slips (block_id, tx_id, slip_id, slip_json_id, golden_ticket, fee_ticket, address, amount, block_hash, longest_chain, spent) VALUES ("+block_id+", "+tx_id+", "+slip_id+", '"+this.lastID+"', '"+slip.gt+"', "+slip.ft+", '"+slip.add+"', "+slip.amt+", '"+bhash+"', "+lc+", 0)");
...
//console.log("SQL CODE executed in "+storage_self.dbname);
      ...
      if (am_i_child == 1) {
        storage_self.slips_saved++;
//console.log("SLIP SAVED++ " + storage_self.slips_saved + " - " + storage_self.slips_to_save);
        if (storage_self.slips_saved == storage_self.slips_to_save) {
          process.send({ slipSaved : storage_self.slips_saved });
          storage_self.saving_slips = 0;
          if (storage_self.spending_slips == 0) { storage_self.execDatabase("COMMIT", {}, function(err) {}); }
...
Storage.prototype.spendSlip = function spendSlip(spending_block_id, block_id, tx_id, slip_id, address, block_hash, am_i_child=0) {
...
//  console.log("UPDATE slips SET spent = "+spending_block_id+" WHERE block_hash = \""+block_hash+"\" AND block_id = "+block_id+" AND tx_id = "+tx_id+" AND slip_id = "+slip_id+" AND address = \""+address+"\"");
  this.execDatabase(sql, params, function(err) {
    ...
    if (am_i_child == 1) {
      ...
      if (storage_self.slips_spent == storage_self.slips_to_spend) {
        process.send({ slipSpent : storage_self.slips_spent });
        if (storage_self.saving_slips == 0) { storage_self.execDatabase("COMMIT", {}, function(err) {}); }
        storage_self.spending_slips = 0;
      }
    } else {
      storage_self.slips_spent++;
    ...
    storage_self.queryBlockchainArray(sql, params, function (err, rows) {
      if (rows.length > 0) {
        for (var x = 0; x < rows.length; x++) {
          var block_filename = __dirname + "/../data/blocks/" + rows[x].block_id + "-" + rows[x].id + ".blk";
          fs.unlink(block_filename, function(error) {
	  });
        }
      }
...
//////////////////////////////////
Storage.prototype.validateInputsWithDualCallbacks = function validateInputsWithDualCallbacks(blk, tx, spent_block_id, unacceptable_block_hashes_sql_list, old_lc, lc_len, forceAdd, success_callback, failure_callback) {
...
Storage.prototype.validateInputsChildProcessSuccess = function validateInputsChildProcessSuccess(storage_self, vt, vs, old_lc, lc_len) {

  storage_self.txs_validated += vt;
  storage_self.slips_validated += vs;

  // all transactions valided, return slips
  if (storage_self.txs_validated >= storage_self.txs_to_validate) {
    process.send({ slipValidated : 1 , block_hash : storage_self.bhash_to_validate , slips_validated : storage_self.slips_validated , forceAdd : "no" , old_lc : old_lc , lc_len : lc_len });
  }

}
Storage.prototype.validateInputsChildProcess = function validateInputsChildProcess(blk, spent_block_id, unacceptable_block_hashes_sql_list, old_lc, lc_len, forceAdd, child_number = 0) {

  var storage_self = this;

  this.txs_to_validate = blk.block.transactions.length;
  this.txs_validated   = 0;
  this.slips_validated = 0;

  if (this.txs_to_validate == 0) {
    storage_self.validateInputsChildProcessSuccess(storage_self,0,0);
  } else {
    for (var i = 0; i < blk.block.transactions.length; i++) {
      storage_self.validateInputsWithDualCallbacksChild(blk, blk.block.transactions[i], spent_block_id, unacceptable_block_hashes_sql_list, old_lc, lc_len, forceAdd, storage_self.validateInputsChildProcessSuccess, storage_self.validateInputsChildProcessFailure, child_number);
    }
  }


}
Storage.prototype.validateInputsWithDualCallbacksChild = function validateInputsWithDualCallbacksChild(blk, tx, spent_block_id, unacceptable_block_hashes_sql_list, old_lc, lc_len, forceAdd, success_callback, failure_callback, child_number = 0) {

  var storage_self = this;

  if (this.app.BROWSER == 1 || this.app.SPVMODE == 1) { success_callback(1, tx.transaction.from.length, old_lc, lc_len); return; }
  ...
  // if we have only golden ticket inputs -- first child reports as success
  if (gtnum == utxiarray.length) { if (child_number == 0) { success_callback(storage_self, 1, gtnum, old_lc, lc_len); } return; }

  this.queryBlockchain(sql, params, function (err, row) {
    if (row != null) {
      if (row.count > 0) {
        if (child_number == 0) {
	  success_callback(storage_self, 1, row.count+gtnum, old_lc, lc_len);
	} else {
	  success_callback(storage_self, 1, row.count, old_lc, lc_len);
	}
      } else {
	if (child_number == 0) {
	  success_callback(storage_self, 1, gtnum, old_lc, lc_len);
	}
      }
    } else {
      success_callback(storage_self, 1, 0, old_lc, lc_len);
    }
  });

  return;
}
Storage.prototype.validateTransactionInputsForAddingToMempool = function validateTransactionInputsForAddingToMempool(tx, mycallback, am_i_child=0) {
  ...
  ////////////////
  //
  // this needs to be re-written so that we actually check the input slips according
  // to where they are stored in our multi-server/CPU setup. For now though we can
  // just accept all mempool submissions as valid.
  //
  if (this.child_forks.length > 0) {
    //console.log("Did not validate TX when adding to mempool ... ");
    mycallback(this.app, tx);
    return;
  }
...
Storage.prototype.validateTransactionInputsOnBlockchainIndexAndStore = function validateTransactionInputsOnBlockchainIndexAndStore(myblk, chain_hashes, old_lc, lc_len, blocks_to_validate_sequentially, forceAdd="no", am_i_child = 0, child_number = 0, max_children = 0) {
  ...
  if (blocks_to_validate_sequentially == 1) {
    ...
    if (this.child_forks.length > 0) {
      ...
      /////////////////////////////////////////////////////////
      // slips_to_validate set in saveBlock which runs first //
      /////////////////////////////////////////////////////////
      this.slips_validated     = 0;
      this.txs_validated       = 0;
      this.block_to_validate   = myblk;
      this.bhash_to_validate   = myblk.hash();
      this.children_validated  = 0;

      var blk_json = JSON.stringify(myblk.block);
      var txs_json = JSON.stringify(myblk.transactions);

      for (var cf = 0; cf < this.child_forks.length; cf++) {
        this.child_forks[cf].send({ 
	  validateBlock: blk_json ,
          txs_json : txs_json ,
	  block_id : myblk.returnId() ,
	  block_hash : this.bhash_to_validate , 
	  child_number : cf , 
	  max_children : this.child_forks.length ,
	  chain_hash_sql : chain_hash_sql , 
	  old_lc : old_lc ,
	  lc_len : lc_len ,
	  forceAdd : forceAdd 
	});
    ...
    storage_self.queryBlockchainArray(sql, params, function (err, rows) {
      ...
      if (rows.length == 0) {
        storage_self.app.blockchain.successfulBlockTransactionValidation(storage_self.app, null, myblk, forceAdd);
        return;
      } else {
        for (var sqlr = 0; sqlr < rows.length; sqlr++) {
	  ...
	  if (this.save_blocks_in_db == 1) {
            ...
            var sql2 = "SELECT * FROM blocks_json WHERE id = $block_json_id";
            var params2 = {
              $block_json_id : rows[sqlr].block_json_id
            } 
            storage_self.queryBlockchain(sql2, params2, function (err2, row2) {

              var tmpblk = new saito.block(this.app, row2.block_json);
	  ...
	  // FROM DISK //
 	  ///////////////
            ...
            var block_filename = __dirname + "/../data/blocks/" + rows[sqlr].block_id + "-" + rows[sqlr].id + ".blk";
	    
	    try {
              fs.readFile(block_filename, 'utf8', (err, data) => {
                ...
                var tmpblk = new saito.block(storage_self.app, data);
                for (var sql2r = 0; sql2r < tmpblk.transactions.length; sql2r++) {
                  var tmptx = tmpblk.transactions[sql2r];
                  storage_self.validateInputsWithDualCallbacks(tmpblk, tmptx, tmpblk.returnId(), chain_hash_sql, old_lc, lc_len, forceAdd, storage_self.app.blockchain.successfulBlockTransactionValidation, storage_self.app.blockchain.failedBlockTransactionValidation);
                }
...
Storage.prototype.indexRecentBlocks = function indexRecentBlocks(mylimit=0) {
  ...
  /////////////////////////////////////////////
  // memory only db = fetch blocks from DISK //
  /////////////////////////////////////////////
  if (this.memory_only_db == 1) {

    var dir = __dirname + "/../data/blocks/";
    ...
    storage_self.reindexing_timer = setInterval(function() {
	  ...
	  return;
        ...
        var block_filename = __dirname + "/../data/blocks/" + files[current_block];
        //var block_filename = "/Users/admin/Programming/saito/lib/data/blocks/" + files[current_block];
...
//console.log("REPOPULATING: adding block to mempool w/ id: "+ blk.returnId() + " -- " + blk.returnHash());
	...
	var block_filename = __dirname + "/../data/blocks/" + block_id + "-" + block_db_id + ".blk";
	try {
        fs.readFile(block_filename, 'utf8', (err, data) => {
  ...
  this.app.storage.queryBlockchainArray(sql, params, function (err, rows) {
    ...
    for (var r = 0; r < rows.length; r++) {
      ...
      if (this.save_blocks_in_db == 1) {
        ...
        var sql3    = "SELECT * FROM blocks_json WHERE id = $json_id";
        var params3 = { $json_id : rows[r].block_json_id };
	    ...
	    peer.sync_sending = 0;
          ...
          for (var rt = 0; rt < rows2.length; rt++) {
	    var rowr = rows2[rt];
            peer.sendRequest("block", rowr.block_json);
            peer.block_sync = rowr.block_id;
          }
	  peer.sync_sending_chunk = 0;
        });
      ...
      ///////////////
        ...
        var block_filename = __dirname + "/../data/blocks/" + rows[r].block_id + "-" + rows[r].id + ".blk";
        try {
	fs.readFile(block_filename, 'utf8', (err, data) => {
