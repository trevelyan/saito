...
function Block(app, blkjson="") {
  ...
  this.block.coinbase         = 0;           // the amount for this block
...
Block.prototype.validate = function validate() {
  ...
  // check if the money released is appropriate
  if (this.validateMonetaryPolicy() == 0) {
    console.log("Block invalid: the monetary policy is wrong");
    return 0;
  }
...
Block.prototype.createBlock = function createBlock(prevblock=null) {
  ...
  if (prevblock != null) {
    this.block.coinbase   = prevblock.coinbase;  // not adjusted unless golden ticket found below
    this.block.treasury   = prevblock.treasury;  // not adjusted unless golden ticket found below
...
Block.prototype.handleGoldenTicket = function handleGoldenTicket() {
  ...
  for (i = 0; i < this.transactions.length; i++) {
    if (this.transactions[i].transaction.gt != null) {
      ...
      this.treasury = this.treasury - (x.miner_share - x.node_share).toFixed(8);
      this.treasury = this.treasury.toFixed(8);
      ...
      this.coinbase = (this.treasury / this.app.blockchain.genesis_period).toFixed(8);
      ...
      //
      console.log("We have adjusted the treasury and the coinbase....");
...
Block.prototype.validateGoldenTicket = function validateGoldenTicket() {
  ...
  if (prevblk == null) {
    console.log("Previous Block is NULL -- skipping light clients need to be able to validate blocks without checking GT information");
    return 1;
  }
  ...
  var txfees_total  = parseFloat(0.0 + prevblk.calculateTransactionFees());
  var txfees_needed = parseFloat(0.0 + prevblk.calculateTransactionFeesNeeded(prevblk.block.prevhash));
  var miner_share   = parseFloat(txfees_needed * prevblk.block.paysplit);
...
console.log("paysplit is wrong...4" );
      ...
      // we also validate our monetary policy
      prev_treasury = prevblk.treasury;
      prev_coinbase = prevblk.coinbase;

      // because we have a golden ticket, we adjust treasury (amount yet to be released)
      //
      // 1. burn fee deducted from treasury
      //
      prev_treasury = prev_treasury - (x.miner_share - x.node_share).toFixed(8);
      ...
      // increases the treasury, but does not immediately affect the coinbase
      //
      console.log("\nValidating COINBASE and TREASURY: ");
      if (prev_treasury != this.treasury) { 
        console.log("Treasury invalid: "+this.treasury+ " -- " + prev_treasury);
        return 0;
      if (prev_coinbase != this.coinbase) { 
        console.log("Coinbase invalid: "+this.coinbase+ " -- " + prev_coinbase);
        return 0;
      }

      console.log("We have successfully adjusted the treasury and the coinbase....");
      console.log("Treasury: "+this.treasury);
      console.log("Coinbase: "+this.coinbase);
