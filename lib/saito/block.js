...
function Block(app, blkjson = '', conf = -1) {
  ...
  this.prevalidated = 0; // set to 1 to forceAdd to blockchain without running callbacks
...
Block.prototype.bundleBlock = function bundleBlock(prevblock = null) {
  ...
  // ///////////////////
  var block_self = this;
  this.calculateReclaimedFunds((reclaimed) => {
    // /////////////////////////////////////////
...
Block.prototype.calculateReclaimedFunds = function calculateReclaimedFunds(mycallback) {
  ...
  if (eliminated_block < 1) {
    ...
    block_self.app.storage.queryDatabase(sql, params, (err, row) => {
      if (row == null) {
      ...
      block_self.app.storage.openBlockByFilename(filename, (storage_self, blk) => {
        var unspent_amt = 0.0;
        ...
        block_self.app.storage.queryDatabase(sql2, params2, (err2, row2) => {
          if (row2 == null) {
            console.log('Error handling monetary policy....');
            process.exit(0);
          }
          ...
          block_self.app.storage.openBlockByFilename(filename2, (storage_self2, blk2) => {
            var gt_found = 0;
