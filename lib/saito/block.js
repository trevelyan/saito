...
function Block(app, blkjson="") {
  ...
  console.log("BLOCK CREATED: "+this.transactions.length+ " -- " + this.block.transactions.length);
...
console.log("BLOCK TRANSACTIONS DO NOT MATCH. First our JSON: ");
console.log(this.block.transactions);
console.log("NOW OUR RAW OBJECTS");
console.log(JSON.stringify(this.transactions));
console.log("MISSING ITEM: ");
for (bm = 0; bm < this.block.transactions.length; bm++) {

  ti = this.block.transactions[bm];
  hm = 0;

  for (bb = 0; bb < this.transactions; bb++) {
    if (JSON.stringify(this.transactions[bb]) == this.block.transactions[bm]) {
      hm = 1;
    }
  }

  if (hm == 1) {
console.log(this.block.transactions[bm]);
  }
...
Block.prototype.addTransaction = function addTransaction(tx) {
  ...
  console.log("Adding TX: " + this.block.transactions.length + " -- " + this.transactions.length);
}
