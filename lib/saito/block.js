...
Block.prototype.handleGoldenTicket = function handleGoldenTicket() {
  ...
  for (i = 0; i < this.transactions.length; i++) {
    if (this.transactions[i].transaction.gt != null) {
      ...
      //
      this.block.treasury = this.block.treasury - (x.miner_share - x.node_share).toFixed(8);
      ...
      //
      console.log("We have adjusted the treasury and the coinbase.... FUTURE BLOCKS SHOULD HAVE: ");
...
console.log("paysplit is wrong...4" );
      ...
      if (prev_treasury != this.block.treasury) { 
        console.log("Treasury invalid: "+this.block.treasury+ " -- " + prev_treasury);
        return 0;
      }
