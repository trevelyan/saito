...
/****
  ...
  if (this.validateGoldenTicket() == 0) {
    ...
    this.app.blockchain.mempool.deleteGoldenTicket();
    return 0;
  }
****/
...
console.log("ROOTHASH: "+this.block.roothash);
  ...
  ///////////////////////////////
  // if our block header is mod of X, adjust difficulty and
  // paysplit, depending on previous votes
  //if ((this.block.header.id%this.app.blockchain.voteperiod) == 0) {
  //  console.log("Adjusting difficulty and paysplit....");
  //}
  // adjust paysplit and difficulty if needed
...
Block.prototype.handleGoldenTicket = function handleGoldenTicket() {
  ...
  for (i = 0; i < this.transactions.length; i++) {
    if (this.transactions[i].transaction.gt != null) {
      ...
      if (paysvote == -1) {
        this.block.header.paysplit -= 0.0001;
        this.block.header.paysplit = this.block.header.paysplit.toFixed(8);
      }
      if (paysvote == 1) {
        this.block.header.paysplit += 0.0001;
        this.block.header.paysplit = this.block.header.paysplit.toFixed(8);
      ...
      if (diffvote == -1) {
        this.block.header.difficulty -= 0.0001;
        this.block.header.difficulty = this.block.header.difficulty.toFixed(8);
      }
      if (diffvote == 1) {
        this.block.header.difficulty += 0.0001;
        this.block.header.difficulty = this.block.header.difficulty.toFixed(8);
...
Block.prototype.validateGoldenTicket = function validateGoldenTicket() {
  ...
  prevblk = this.app.blockchain.returnBlockByHash(this.block.header.prevhash);

//console.log("validating: "+this.hash());
//console.log("prevblk  : "+prevblk.hash());
//console.log("thisblkph: "+this.block.header.prevhash);
