...
function Block(app, blkjson="", conf=-1) {
  ...
  this.block.fee_step         = 0.0000165;
  this.block.difficulty       = 1.0;
...
Block.prototype.validate = function validate() {
  ...
  //
  if (this.block.transactions.length > 0) {
    if (this.areTransactionFeesAdequate() == 0) {
  ...
  for (zz = 0; zz < this.transactions.length; zz++) {
    ...
    if (this.transactions[zz].validateInputs(this.app, this) != 1) {
      console.log("Block invalid: contains invalid inputs");
      console.log(this.transactions[zz].transaction);
      return 0;
    }
  }
...
Block.prototype.returnGoldenTicketContenders = function returnGoldenTicketContenders() {
  ...
  for (v = 0; v < this.transactions.length; v++) {
    for (x = 0; x < this.transactions[v].transaction.to.length; x++) {
      children.push(this.transactions[v].transaction.to[x].add);
    }
  }
...
Block.prototype.calculateBurnFee = function calculateBurnFee(starting_burn_fee, starting_fee_step) {
  ...
  console.log("PBUT: "+prevblk_unixtime+" -- " + this.block.prevhash);
  ...
  // if we are slower than normal
  console.log("\n\nHANDLING BLOCKTIME: ");
  console.log("HEARTBEAT: "+target_time + " -- " + "TIME: " + block_time);

  // faster than target
  if (target_time > block_time) {

    bf[0] += 0.0001;
    bf[1]  = bf[0] / this.app.blockchain.max_heartbeat
    bf[1]  = bf[1].toFixed(8);

    console.log("RAISING FEE AND FEE_STEP: ");
    console.log(bf[0] + " -- " + bf[1]);
    ...
    console.log("LOWERING FEE AND FEE_STEP: ");
    console.log(bf[0] + " -- " + bf[1]);
