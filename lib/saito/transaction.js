...
function Transaction(txjson="") {
  ...
  if (txjson != "") {
    ...
    for (i = 0; i < this.transaction.from.length; i++) {
      this.transaction.from[i] = new saito.slip(this.transaction.from[i].add, this.transaction.from[i].amt, this.transaction.from[i].gt);
    }
    for (i = 0; i < this.transaction.to.length; i++) {
      this.transaction.to[i] = new saito.slip(this.transaction.to[i].add, this.transaction.to[i].amt, this.transaction.from[i].gt);
    }
  }
...
Transaction.prototype.validate = function validate() {
  ...
  if (!saito.crypt().verifyMessage(JSON.stringify(this.transaction.msg),this.transaction.msig,this.transaction.from[0].returnAddress())) {
...
Transaction.prototype.returnSlipsTo = function returnSlipsTo(toAddress) {
  ...
  for (v = 0; v < this.transaction.to.length; v++) {
    if (this.transaction.to[v].returnAddress() == toAddress) { x.push(this.transaction.to[v]); }
  }
  return x;
}
Transaction.prototype.returnSlipsFrom = function returnSlipsFrom(fromAddress) {
  ...
  for (v = 0; v < this.transaction.from.length; v++) {
    if (this.transaction.from[v].returnAddress() == fromAddress) { x.push(this.transaction.from[v]); }
  }
  return x;
}
