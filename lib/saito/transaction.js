...
Transaction.prototype.validate = function validate(app, paysplit_vote=0, block_id=0) {
  ...
  for (var tidx = 0; tidx < this.transaction.from.length; tidx++) {
    if (this.transaction.from[tidx].bid < acceptable_lower_block_limit && this.transaction.ft != 1) {
      console.log("transaction invalid: tries to spend input from block "+this.transaction.from[tidx].bid);
...
console.log("VERIFICATION MESS: "+this.returnSender() + " --- " + this.transaction.sig + " ---- " + this.signatureSource());
