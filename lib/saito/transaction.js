...
Transaction.prototype.debugHTML = function debugHTML() {
  ...
  html += '<tr><td>From</td><td>'+JSON.stringify(this.transaction.from)+'</td></tr>';
  html += '<tr><td>To</td><td>'+JSON.stringify(this.transaction.to)+'</td></tr>';
  ...
  html += '<tr><td>Path</td><td>'+JSON.stringify(this.transaction.path)+'</td></tr>';
  if (this.transaction.gt != "") {
    html += '<tr><td>Golden Ticket</td><td>'+JSON.stringify(this.transaction.gt)+'</td></tr>';
  } else {
    html += '<tr><td>Golden Ticket</td><td>'+this.transaction.gt+'</td></tr>';
  }
  html += '<tr><td>Message</td><td>'+JSON.stringify(this.transaction.msg)+'</td></tr>';
...
Transaction.prototype.validateInputsWithCallbackOnSuccess = function validateInputsWithCallbackOnSuccess(app, mycallback) {
  ...
  if (this.transaction.from == null) { return; }
  for (utxii = 0; utxii < this.transaction.from.length; utxii++) {
    app.storage.validateInputWithCallbackOnSuccess(tx, this.transaction.from[utxii], function(app, tx) {
      mycallback(app, tx);
    });
  }
