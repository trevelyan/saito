...
console.log(this.transaction.from);
      this.transaction.from[i] = new saito.slip(this.transaction.from[i].add, this.transaction.from[i].amt, this.transaction.from[i].gt, this.transaction.from[i].bid, this.transaction.from[i].tid, this.transaction.from[i].sid, this.transaction.from[i].bhash, this.transaction.from[i].lc);
    }
    for (i = 0; i < this.transaction.to.length; i++) {
      this.transaction.to[i] = new saito.slip(this.transaction.to[i].add, this.transaction.to[i].amt, this.transaction.to[i].gt, this.transaction.to[i].bid, this.transaction.to[i].tid, this.transaction.to[i].sid, this.transaction.to[i].bhash, this.transaction.to[i].lc);
    }
  }
...
Transaction.prototype.validate = function validate() {
  ...
  if (!saito.crypt().verifyMessage(this.paymentSignatureSource(),this.transaction.sig,this.transaction.from[0].add)) {
    console.log("transaction signature does not verify");
    return 0;
  }

  if (!saito.crypt().verifyMessage(this.transaction.msg,this.transaction.msig,this.transaction.from[0].add)) {
