...
Crypt.prototype.signMessage = function signMessage(msg, privkey) {
  return secp256k1.sign(Buffer.from(this.hash(JSON.stringify(msg)),'hex'), Buffer.from(privkey,'hex')).signature.toString('hex');
}
Crypt.prototype.verifyMessage = function verifyMessage(msg, sig, pubkey) {
  return secp256k1.verify(Buffer.from(this.hash(JSON.stringify(msg)),'hex'), Buffer.from(sig,'hex'), Buffer.from(this.uncompressPublicKey(pubkey),'hex'));
}
