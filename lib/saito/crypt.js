...
Crypt.prototype.signMessage = function signMessage(msg, privkey) {
  // forced conversion to base-64 avoids cross-platform crypto library issues
  msg = Buffer.from(msg, 'utf-8').toString('base64');
  return secp256k1.sign(Buffer.from(this.hash(JSON.stringify(msg.toString('base64'))),'hex'), Buffer.from(privkey,'hex')).signature.toString('hex');
...
Crypt.prototype.verifyMessage = function verifyMessage(msg, sig, pubkey) {
  // forced conversion to base-64 avoids cross-platform crypto library issues
  msg = Buffer.from(msg, 'utf-8').toString('base64');
  return secp256k1.verify(Buffer.from(this.hash(JSON.stringify(msg.toString('base64'))),'hex'), Buffer.from(sig,'hex'), Buffer.from(this.uncompressPublicKey(pubkey),'hex'));
