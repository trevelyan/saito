...
Crypt.prototype.signMessage = function signMessage(msg, privkey) {
  var x = this.toBase58(secp256k1.sign(Buffer.from(this.hash(Buffer.from(msg, 'utf-8').toString('base64')),'hex'), Buffer.from(privkey,'hex')).signature.toString('hex'));
//console.log("\n\nSIGN: "+msg.substring(0,1000));
//console.log("hash: "+x);
  return x;
}
Crypt.prototype.verifyMessage = function verifyMessage(msg, sig, pubkey) {
  var x = secp256k1.verify(Buffer.from(this.hash(Buffer.from(msg, 'utf-8').toString('base64')),'hex'), Buffer.from(this.fromBase58(sig),'hex'), Buffer.from(this.uncompressPublicKey(pubkey),'hex'));
//console.log("\n\nVERIFY: "+msg.substring(0,1000));
//console.log("sig:  "+sig);
//console.log("hash: "+x);
  return x;
}
...
// Merkle Trees //
//////////////////
Crypt.prototype.merkleTree = function merkleTree(inarray) {
