...
Wallet.prototype.resetSpentInputs = function resetSpentInputs() {
  for (var wi = 0; wi < this.wallet.utxi.length; wi++) { this.spent_slips[wi] = 0; }
}
...
console.log("INADEQUATE INPUTS: ");
console.log(this.wallet.utxi);
console.log("SPENT INPUTS: ");
console.log(this.spent_slips);
...
Wallet.prototype.paymentConfirmation = function paymentConfirmation(blk, tx, i_am_the_longest_chain) {
  ...
  if (tx.isTo(this.returnPublicKey())) {
    ...
    for (var ms = 0; ms < my_slips.length; ms++) {
	  ...
	  // make sure longest chain is marked as needed, even 
	  // if we are trying to re-insert. this can happen if
	  // we have a partial sync from later in the chain 
	  // -- older slips marked as non-LC as there is a longer
	  // initial chain found earlier, but early slips not
	  // re-added because they are duplicates
	  for (var x = 0; x < this.wallet.utxi.length; x++) {
	    var tmpslip = this.wallet.utxi[x];
	    if (tmpslip.bhash == this_slip.bhash && tmpslip.bid == this_slip.bid && tmpslip.tid == this_slip.tid && tmpslip.sid == this_slip.sid && i_am_the_longest_chain == 1) {
	      this.wallet.utxi[x].lc = i_am_the_longest_chain;
	    }
	  }
