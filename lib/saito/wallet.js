...
function Wallet(app, walletjson="") {
  ...
  this.wallet.utxi               = [];
  this.wallet.utxo               = [];
  this.wallet.version            = 1.45;

  ///////////////////
  // Bloom Filters //
  ///////////////////
  this.utxibloom                 = new saito.bloom(8 * 1024 * 1024, 16, 0xdeadbee0);
  this.utxobloom                 = new saito.bloom(8 * 1024 * 1024, 16, 0xdeadbee0);
  this.utxibloom_limit   	 = 10000;  // rewrite the bloom filters after how many inserts ?
  this.utxibloom_counter 	 = 0;
...
/*
1. test the array back to the point the BID is < than ours
2. insert at that point

this gives us a BID organized array even in cases of 
chain re-writes. most wallets should handle the syncing
rather quickly.
*/

  var pos = this.wallet.utxi.length;
  var tgt = x.bid-1;
  while (pos > 0 && this.wallet.utxi[pos-1].bid > tgt) {
    pos--;
  }
  if (pos == -1) { pos = 0; }
...
/*
 * OLD CODE -- not needed hopefully, but keeping it
 *
 * here until we can confirm the ordered insert is not 
 * incredibly slow
 *
  // sanity check -- do not add if already exists in wallet
  // can be triggered in rare cases where SPV nodes reload
  // the blockchain from a block that contains an incoming 
  // payment, since we don't detect the block as a duplicate
  // as the hash is not indexed in the blockchain class when
  // adding the block
  var latest_wallet_bid = 0;
  if (this.wallet.utxi.length > 0) {
    if (latest_wallet_bid == 0) {
      latest_wallet_bid = this.wallet.utxi[this.wallet.utxi.length-1].bid;
    }
  }

  // transactions will be organized in ascending order by BID
  // so we can avoid checking the entire transaction history
  // if the incoming slip has a larger BID than our largest
  // bid
  //
  // this will probably create problems in the event of 
  // chain reorganizations, if the inbound payment precedes
  // the latest block_id but is on a fork that is more than
  // fork_guard behind
  //
  if (x.bid <= latest_wallet_bid) {
    for (var i = this.wallet.utxi.length-1; i >= 0 && x.bid > (latest_wallet_bid - 20); i--) {
      if (this.wallet.utxi[i].bhash == x.bhash) {
        if (this.wallet.utxi[i].amt == x.amt && this.wallet.utxi[i].bid == x.bid && this.wallet.utxi[i].tid == x.tid && this.wallet.utxi[i].sid == x.sid) {
          if (x.lc == 1 && this.wallet.utxi[i].lc == 0) {
            this.wallet.utxi[i].lc = 1;
          }
          return;
        }
      }
 ...
 */
  ...
  this.utxibloom_counter++;
  if (this.utxibloom_counter > this.utxibloom_limit) {

    console.log("Recreating Wallet BLOOM Filters");
    this.utxibloom = new saito.bloom(8 * 1024 * 1024, 16, 0xdeadbee0);
    this.utxobloom = new saito.bloom(8 * 1024 * 1024, 16, 0xdeadbee0);
    for (var x = 0; x < this.wallet.utxi.length; x++) {
      this.utxibloom.add(this.app.crypt.hash(JSON.stringify(this.wallet.utxi[x])), 'hex');
    }
    for (var z = 0; z < this.wallet.utxo.length; z++) {
      this.utxobloom.add(this.app.crypt.hash(JSON.stringify(this.wallet.utxo[z])), 'hex');
    }
    console.log("Recreated Wallet BLOOM Filters!");
    this.utxibloom_counter = 0;
  } else {
    this.utxibloom.add(this.app.crypt.hash(JSON.stringify(x)), 'hex');
  }
}

Wallet.prototype.addOutput = function addOutput(x) {
//console.log(" ... WALLET: adding output.");
  ...
  this.utxobloom.add(this.app.crypt.hash(JSON.stringify(x)), 'hex');

  this.utxo_storage_counter++;

  if (this.utxo_storage_counter >= this.utxo_storage_limit) {

    // delete excessive UTXO inputs
    // this can hypothetically cause problems but is needed
    // for wallets that spend a tremendous amount as otherwise
    // our UTXO spends just grow in the options file uncontrollably
...
Wallet.prototype.getAvailableInputs = function getAvailableInputs(limit=0) {
...
Wallet.prototype.paymentConfirmation = function paymentConfirmation(blk, tx, lchain) {
  ...
  // all the array searching becomes a problem when processing 100s to 1000s of 
  // inbound or outbound payments. should we optimize this by using a bloom 
  // filter? or will false errors screw up our wallet?
      ...
      if (s.amt > 0) { 
	    ...
	    for (var x = 0; x < this.wallet.utxi.length; x++) {
	      ...
	      if (t.bhash == s.bhash && t.bid == s.bid && t.tid == s.tid && t.sid == s.sid && lchain == 1) {
	        ...
	        x = this.wallet.utxi.length;
	      }
	    }

	    // adding this lets us just use the bloom filter in containsUtxi, since
	    // any misreported slips will still get added.
	    if (slip_updated == 0) {
	      if (this.containsUtxo(s) == 0) {
	        this.addInput(s);
	        //this.wallet.balance = this.calculateBalance();
...
// false positive is not considered dangerous since we can 
// code around it by looping through for update and only
// catching if the entry does not exist.
Wallet.prototype.containsUtxi = function containsUtxi(s) {

  // just check against BLOOM filter for maximum efficiency
  //
  // uncomment section below to search in case we get a 
  // positive match and need to confirm that it is not
  // a false positive.
  if (this.utxibloom.test(JSON.stringify(s), 'hex') == false) {
    return 0;
  }
  return 1;

  /**************
  for (var x = this.wallet.utxi.length-1; x >= 0; x--) {
...
Wallet.prototype.containsUtxo = function containsUtxo(s) {
  ...
  if (this.utxobloom.test(JSON.stringify(s), 'hex') == false) {
    return 0;
  }
