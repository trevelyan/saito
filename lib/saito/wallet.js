...
console.log(tmp_genesis_id + " -- " + this.wallet.utxi.length);
...
console.log(this.wallet.utxi[i]);
...
console.log("\n\nTX INPUT");
console.log("\n\n"+total_fees);
console.log(this.returnBalance());
...
Wallet.prototype.handleChainReorganization = function handleChainReorganization(block_id, block_hash, lc) {
  ...
  console.log("\n\nHANDLING CHAIN REORGANIZATION IN WALLET: " + lc + " -- " + block_hash);

  for (mv = this.wallet.utxi.length-1; mv  >= 0; mv--) {
    //
    // before we uncomment, confirm that UXTI are added in BID order
    // so that earlier BID will not be ignored if they are at the
    // beginning -- chain reorganizations could screw this up
    //
