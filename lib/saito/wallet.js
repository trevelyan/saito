...
console.log("GETTING ADEQUATE INPUTS: "+amount_needed);
console.log("WALLET LENGTH: "+this.wallet.utxi.length);

  for (i = 0; i < this.wallet.utxi.length; i++) {
    utxiset.push(this.wallet.utxi[i]);
    value += this.wallet.utxi[i].returnAmount();
...
Wallet.prototype.createUnsignedTransactionWithFee = function createUnsignedTransactionWithFee(to, amt, fee) {
  ...
  //
  tx.transaction.from = this.getAdequateInputs(amt+fee);
...
Wallet.prototype.createGoldenTransaction = function createGoldenTransaction(winners, mysolution) {
  ...
  tx.transaction.msig         = this.signMessage(JSON.stringify(tx.transaction.msg));
console.log("AAAAAAA");
console.log(tx.paymentSignatureSource());
  tx.transaction.sig          = this.signMessage(tx.paymentSignatureSource());
console.log("BBBBBBB");
...
Wallet.prototype.paymentConfirmation = function paymentConfirmation(blk, tx) {
  ...
  if (tx.isTo(this.returnPublicKey())) {
    ...
    for (ms = 0; ms < my_slips.length; ms++) {
      ...
      this_slip.block_id       = blk.block.id;
      this_slip.transaction_id = tx.transaction.id;
      this_slip.slip_id        = ms+1;
	...
	console.log("\nPAYMENT ADDED TO WALLET, new BALANCE: ");
  ...
  if (tx.isFrom(this.returnPublicKey())) {
    ...
    for (ms = 0; ms < my_slips.length; ms++) {
      ...
      for (sc = 0; sc < this.wallet.utxi.length; sc++) {
        ...
        if (
			this_slip.block_id == qs.block_id &&
			this_slip.transaction_id == qs.transaction_id &&
			this_slip.slip_id == qs.slip_id &&
	  ...
	  sc = this.wallet.utxi.length+1;
          console.log("\n\n\nPAYMENT SPENT and REMOVED from WALLET: ");
          console.log(this_slip);
	}
      }
    }
