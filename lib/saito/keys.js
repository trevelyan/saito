...
Keys.prototype.addKey = function addKey(publickey, identifier, watch=0, keyword2watch="") {
  ...
  for (xmn = 0; xmn < this.keys.length; xmn++) {
    if (this.keys[xmn].publickey == publickey) {
      if (this.keys[xmn].identifier != identifier) {
	this.keys[xmn].identifier = identifier;
	this.keys[xmn].watch = watch;
	this.keys[xmn].keyword = keyword2watch;
        return; 
      }
    }
  }
...
Keys.prototype.updateCryptoByIdentifier = function updateCryptoByIdentifier(identifier, cryptoobj) {
  ...
  for (xmn = 0; xmn < this.keys.length; xmn++) {
    if (this.keys[xmn].identifier == identifier) { 
      ...
      if (cryptoobj != null) {
        ...
        this.keys[xmn].aes_ss      = cryptoobj.aes_ss;
        this.keys[xmn].aes_privkey = cryptoobj.aes_privkey;
        this.keys[xmn].aes_pubkey  = cryptoobj.aes_pubkey;
...
Keys.prototype.updateCryptoByPublicKey = function updateCryptoByPublicKey(publickey, cryptoobj) {
  ...
  this.addKey(publickey, "", 0, "");
  this.updateCryptoByPublicKey(publickey, cryptoobj);
  this.saveKeys();
...
Keys.prototype.initializeKeyExchange = function initializeKeyExchange(tx) {

  to = tx.transaction.to[0].add;
  ...
  cryptoobj = {};
  cryptoobj.aes_ss      = "";
  cryptoobj.aes_pubkey  = alice_publickey;
  cryptoobj.aes_privkey = alice_privatekey;

  this.updateCryptoByPublicKey(to, cryptoobj);
