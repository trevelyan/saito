...
GoldenTicket.prototype.validateMonetaryPolicy = function validateMonetaryPolicy(adjusted_treasury, adjusted_coinbase, prevblock) {
  ...
  //console.log("Adjusted Treasury: " + monetary_policy[0] + " -- " + adjusted_treasury);
  //console.log("Adjusted Coinbase: " + monetary_policy[1] + " -- " + adjusted_coinbase);
...
GoldenTicket.prototype.calculateMonetaryPolicy = function calculateMonetaryPolicy(prevblk) {
  ...
  prev_treasury = parseFloat(prev_treasury) + parseFloat(prev_reclaimed);
  prev_treasury = prev_treasury - this.solution.miner_share;
  prev_treasury = prev_treasury - this.solution.node_share;
  prev_treasury = prev_treasury.toFixed(8);

console.log("calculating cb: "+prev_treasury+" -- " + prevblk.app.blockchain.genesis_period);
