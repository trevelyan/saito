...
GoldenTicket.prototype.createSolution = function createTicket(block_to_solve, solution_public_key, solution_private_key) {
    ...
    var txfees_total  = parseFloat(0.0 + block_to_solve.calculateUsableTransactionFees()).toFixed(8);
...
GoldenTicket.prototype.validate = function validate(prevblk, thisblk) {
  ...
  var txfees_total  = parseFloat(0.0 + prevblk.calculateUsableTransactionFees()).toFixed(8);
...
GoldenTicket.prototype.validateMonetaryPolicy = function validateMonetaryPolicy(adjusted_treasury, adjusted_coinbase, prevblock) {
  ...
  monetary_policy = this.calculateMonetaryPolicy(prevblock);
...
GoldenTicket.prototype.calculateMonetaryPolicy = function calculateMonetaryPolicy(prevblk) {
  ...
  prev_treasury = prevblk.returnTreasury();
  prev_coinbase = prevblk.returnCoinbase();
  prev_treasury = prev_treasury - this.solution.miner_share - this.solution.node_share;
  ...
  // TODO -- HACK
  //
  // ideally we want to track monetary policy and only add the amount
  // of tokens needed to the network. Since we have no idea how many 
  // are being lost without measuring the golden tickets and unspent
  // inputs, we take a shortcut this version and just add some extra
  // cash to the treasury every block.
  prev_treasury = prev_treasury + 5;
