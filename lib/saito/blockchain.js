...
Blockchain.prototype.debugHTML = function debugHTML() {
  ...
  html += '<tr><th></th><th>Block Id</th><th>Block Hash</th><th>Previous Block Hash</th></tr>';

  for (mb = this.blocks.length-1; mb >= 0 && mb > this.blocks.length-50; mb--) {
    html += '<tr>';
    longestchainhash = this.index.hash[this.longestChain];
    if (longestchainhash == this.blocks[mb].hash()) {
      html += '<td>***</td><td><a href="/info/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].block.id+'</a></td><td><a href="/info/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].hash()+'</a></td><td>'+this.blocks[mb].block.prevhash+'</td>';
    } else {
      html += '<td></td><td><a href="/info/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].block.id+'</td><td><a href="/info/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].hash()+'</a></td><td>'+this.blocks[mb].block.prevhash+'</td>';
    }
    html += '</tr>';
  }
  html += '</table>';
...
Blockchain.prototype.deIndexAndPurge = function deIndexAndPurge(blk) {
  ...
  // resume mining //
  ///////////////////
  this.miner.startMining(prevblock);
