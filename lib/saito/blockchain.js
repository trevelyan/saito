...
//  console.log("\nHASHES\n");
//  console.log(this.index.hash);

//  console.log("\nPREV HASHES\n");
//  console.log(this.index.prevhash);

  console.log("\n\n");
  for (mb = 0; mb < this.blocks.length; mb++) {
    longestchainhash = this.index.hash[this.longestChain];
    if (longestchainhash == this.index.hash[mb]) {
...
Blockchain.prototype.returnLongestChain = function returnLongestChain(chainlength=10) {
  ...
  for (i = chainarray.length-1; i >= 0 && finished == 0; i--) {
    tmpblk = this.returnBlockByHash(this.index.hash[our_longest_chain[i]]);
    if (tmpblk != null) {
