...
Blockchain.prototype.debugHTML = function debugHTML() {
  ...
  for (mb = this.blocks.length-1; mb >= 0 && mb > this.blocks.length-50; mb--) {
    ...
    longestchainhash = this.index.hash[this.longestChain];
    if (longestchainhash == this.blocks[mb].hash()) {
      html += '<td>*</td><td><a href="/info/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].block.id+'</a></td><td><a href="/info/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].hash()+'</a></td><td>'+this.blocks[mb].block.prevhash.substring(0,15)+'...</td>';
    } else {
