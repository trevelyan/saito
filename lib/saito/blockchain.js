...
Blockchain.prototype.addBlockToBlockchain = function addBlockToBlockchain(newblock, forceAdd="no") {
  ...
  var rewrite_shared_ancestor_index_pos = -1;
  ...
  if (block_id >= this.index.block_id[this.longestChain]) {
	...
	rewrite_shared_ancestor_index_pos = shared_ancestor_index_pos;
  ...
  if (	blockchain_self.app.storage.saving_blocks == 1 || blockchain_self.app.storage.saving_slips == 1 || blockchain_self.app.storage.spending_slips == 1) {
    blockchain_self.block_saving_timer = setInterval(function() {
      blockchain_self.addBlockToBlockchainPartTwo(newblock, pos, validate_transactions, hash, existing_longestChain, forceAdd,  rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, i_am_the_longest_chain);
...
// some reason.
//
Blockchain.prototype.addBlockToBlockchainPartTwo = function addBlockToBlockchainPartTwo(newblock, pos, validate_transactions, hash, existing_longestChain, forceAdd,  rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, i_am_the_longest_chain) {
  ...
  if (validate_transactions != -1) {
    ...
    //
    this.validateLongestChain(newblock, rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, forceAdd);
...
Blockchain.prototype.validateLongestChain = function validateLongestChain(newblock, rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, rewrite_forceAdd) {
  ...
  if (rewrite_shared_ancestor_index_pos > -1) {
    ...
    for (var x = new_block_idxs.length-1; x >= 0; x--) {
      ...
      if (this.app.storage.onChainReorganization(this.index.block_id[x], this.index.hash[x], 1);
 == -1) {
