...
function Blockchain(app) {
  ...
  // Callbacks //
  ///////////////
  this.callback_limit = 10;        // only run callbacks on the last X blocks
  ...
  // 1 week blockchain, 30 second blocks = 20160
  this.max_heartbeat           = 60;         // burn fee hits zero every 120 seconds
  this.genesis_period          = 21000;      // number of blocks before money disappears
...
Blockchain.prototype.debugHTML = function debugHTML() {
  ...
  for (var mb = this.blocks.length-1; mb >= 0 && mb > this.blocks.length-200; mb--) {
    ...
    var longestchainhash = this.index.hash[this.longestChain];
    if (longestchainhash == this.blocks[mb].hash()) {
      html += '<td>*</td><td><a href="/explorer/block?hash='+this.blocks[mb].hash('hex')+'">'+this.blocks[mb].block.id+'</a></td><td><a href="/explorer/block?hash='+this.blocks[mb].hash('hex')+'">'+this.blocks[mb].hash()+'</a></td><td>'+this.blocks[mb].block.prevhash.substring(0,25)+'...</td>';
    } else {
      html += '<td></td><td><a href="/explorer/block?hash='+this.blocks[mb].hash('hex')+'">'+this.blocks[mb].block.id+'</td><td><a href="/explorer/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].hash()+'</a></td><td>'+this.blocks[mb].block.prevhash.substring(0,25)+'...</td>';
    }
    html += '</tr>';
  }
  html += '</table>';
...
Blockchain.prototype.indexAndStore = function indexAndStore(newblock, forceAdd="no") {
  ...
  var hash           = newblock.block_hash = newblock.hash('hex');
  ...
  // block unless its timestamp is too far back
  if (prevhash != "") {
...
//console.log(" BC: lbh: " + this.app.options.blockchain.latest_block_hash);

//console.log(" ... inserting indexes");
...
//console.log(" ... track the longest chain");
...
//console.log("SHARED ANCESTOR INDEX POS: " + shared_ancestor_index_pos + " ("+lchain_len+"/"+nchain_len+")");
    ...
    if (newblock.block.prevhash == this.previous_block_hash) {
      ...
      i_am_the_longest_chain = 1;
      this.previous_block_hash = newblock.hash();
      this.longestChain = pos;
      this.app.modules.updateBalance();     
...
//console.log(" ... save block");
  ...
  if (forceAdd != "force" || newblock.thisBlockId > 0) {
...
console.log(" ... utxi/utxo lengths: " + this.app.wallet.wallet.utxi.length + " / " + this.app.wallet.wallet.utxo.length);
...
console.log(" ... position? " + pos);
  ...
  /////////////////////////////////////////////
  if (forceAdd != "force") {
    var our_longest_chain = this.returnLongestChainIndex(this.callback_limit);
    for (var i = 0; i < our_longest_chain.length && i < this.callback_limit; i++) {
...
//console.log(" ... delete transactions from memory");
  ...
  if (this.blocks.length > this.callback_limit) {
    var blk2clear = this.blocks.length - this.callback_limit-1;
    if (blk2clear >= 0) {
      this.blocks[blk2clear].transactions = [];
      this.blocks[blk2clear].block.transactions = [];
      // sanity check for blocks added earlier
      if (pos < blk2clear) {
        this.blocks[pos].transactions = [];
        this.blocks[pos].block.transactions = [];
      }
    }
  }
...
//console.log(" ... update fork and genesis");
  ...
  // update fork id //
  ////////////////////
  if (newblock.returnId()%this.fork_id_mod == 0) { this.updateForkId(newblock); }
...
console.log(" ... UPDATING GENESIS BLOCK: " + pos);
    pos = this.updateGenesisBlock(newblock, pos);
//console.log("UPDATING GENESIS BLOCK: " + pos);
  }
  ...
  this.block_saving_timer = setInterval(function(){

    if (blockchain_self.app.storage.saving_blocks == 1 || blockchain_self.app.storage.saving_txs == 1 || blockchain_self.app.storage.saving_slips == 1 || blockchain_self.app.storage.spending_slips == 1) {
      console.log("NOT DONE SAVING: " + blockchain_self.app.storage.saving_blocks + " " + blockchain_self.app.storage.saving_txs + " " + blockchain_self.app.storage.saving_slips + " " + blockchain_self.app.storage.spending_slips);
    } else {

      // we don't add forced blocks to the database, as we are
      // already reading them in from local storage
      if (forceAdd != "force") {
        blockchain_self.app.storage.execDatabase("COMMIT", {}, function(err) {});
      }
      ...
      /////////////////////////////////
      if (validate_transactions == -1 || newblock.transactions.length == 0) {
        ...
        if (blockchain_self.app.BROWSER == 1 || blockchain_self.app.SPVMODE == 1) {
          ...
          blockchain_self.successfulBlockTransactionValidation(blockchain_self.app);
        } else {
console.log(" ... and validating inputs");
          blockchain_self.app.storage.validateTransactionInputsOnBlockchainIndexAndStore(newblock, nbhasharr, existing_longestChain, 2, validate_transactions, forceAdd);
        }
      }
...
Blockchain.prototype.blockFullyValidated = function blockFullyValidated(app, tx, blk, forceAdd) {
...
Blockchain.prototype.successfulBlockTransactionValidation = function successfulBlockTransactionValidation(app, tx=null, blk, forceAdd) {
  app.blockchain.validate_current++;
  if (app.blockchain.validate_current >= app.blockchain.validate_total) {
    console.log(" ... fully validated: "+app.blockchain.validate_current+"/"+app.blockchain.validate_total);
    app.blockchain.blockFullyValidated(app, tx, blk, forceAdd);
  }
}
...
console.log(" ... revert index to 0 for old LC: " + shared_ancestor_index_pos);
...
console.log(" ... revert index to 1 for new LC: " + shared_ancestor_index_pos);
  }
}
...
Blockchain.prototype.purgeArchivedData = function purgeArchivedData(lowest_block_id, pos) {
  ...
  //
  // note that we are setting items in this array
  // to a new array after our callback limit is hit
  // so even if we have older blocks for reference
  // purpose we cannot necessarily use them.
...
console.log("DELETING FROM DATABASE: " + lowest_block_id);
  this.app.storage.deleteBlocks(lowest_block_id);
console.log("DELETED FROM DATABASE: " + lowest_block_id);
console.log("PURGED: "+ lowest_block_id + " - " + this.returnLatestBlockId());




  ///////////////////////////////
  // and delete from my wallet //
  ///////////////////////////////
  this.app.wallet.purgeExpiredSlips();
...
Blockchain.prototype.returnLatestBlock = function returnLatestBlock() {
  ...
  for (var i = this.blocks.length-1; i >= 0; i--) {
    if (this.blocks[i].block_hash == this.index.hash[this.longestChain]) {
...
Blockchain.prototype.returnBlockByHash = function returnBlockByHash(hash) {
  ...
  for (var v = this.blocks.length-1; v >= 0; v-- ) {
    if (this.blocks[v].block_hash == hash) {
