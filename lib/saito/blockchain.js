...
function Blockchain(app) {
  ...
  // 1 week blockchain, 30 second blocks
  this.max_heartbeat           = 120;       // burn fee hits zero every 120 seconds
  this.genesis_period          = 20160;       // number of blocks before money disappears
  this.fork_guard              = 40;        // discard forks that fall N blocks behind
...
Blockchain.prototype.debugHTML = function debugHTML() {
  ...
  for (var mb = this.blocks.length-1; mb >= 0 && mb > this.blocks.length-200; mb--) {
    ...
    var longestchainhash = this.index.hash[this.longestChain];
    if (longestchainhash == this.blocks[mb].hash()) {
      html += '<td>*</td><td><a href="/explorer/block?hash='+this.blocks[mb].hash('hex')+'">'+this.blocks[mb].block.id+'</a></td><td><a href="/explorer/block?bid='+this.blocks[mb].block.id+'">'+this.blocks[mb].hash()+'</a></td><td>'+this.blocks[mb].block.prevhash.substring(0,25)+'...</td>';
    } else {
...
Blockchain.prototype.indexAndStore = function indexAndStore(newblock, forceAdd="no") {
  ...
  console.log("\n\n\nAdding block "+newblock.returnId() + " -> " + newblock.hash() + " " + newblock.block.unixtime);
...
console.log(" ... payment confirmation");
  ...
  for (var ti = 0; ti < newblock.transactions.length; ti++) {
    ...
    if (tx.isFrom(this.app.wallet.returnPublicKey()) || tx.isTo(this.app.wallet.returnPublicKey())) {
      updated_wallet = 1;
console.log(" ... ADDING PAYMENT: "+ newblock.returnId() + " ... " + i_am_the_longest_chain + " ..." + newblock.hash());
...
console.log("\n\n ------------- UPDATE GENESIS BLOCK --------------- ");
  ...
  if (blk.returnId() >= (this.genesis_block_id + this.genesis_period + this.fork_guard)) {
    ...
    this.purgeArchivedData(purge_id) ;
