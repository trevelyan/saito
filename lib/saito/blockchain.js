...
function Blockchain(app) {
  ...
  this.genesis_period          = 2;         // number of blocks before money disappears
  this.genesis_ts              = 0;         // unixtime of earliest block
  this.genesis_block_id        = 0;         // earlier block_id we care about
  //this.fork_guard              = 120;       // discard forks that fall N blocks behind
  this.fork_guard              = 0;       // discard forks that fall N blocks behind
...
console.log(JSON.stringify(this.blocks[blkpos].transactions, null, 4));
...
console.log("SAVING BLOCK "+newblock.returnId() + " as LC " + i_am_the_longest_chain);
  ...
  if (validate_transactions == -1) {
    ...
    if (this.app.BROWSER == 1 || this.app.SPVMODE == 1) {
      ...
      this.app.storage.validateTransactionInputs(newblock, nbhasharr, this.longestChain, 2, validate_transactions);
    }
  }
...
console.log("SUCCESS IN VALIDATING!");
...
Blockchain.prototype.rewriteLongestChainIndex = function rewriteLongestChainIndex(shared_ancestor_index_pos, correct_pos_array) {
  ...
  for (var h = shared_ancestor_index_pos; h < this.index.lc.length; h++) {
    this.index.lc[h] = 0;
console.log("\n\nUPDATING LONGEST CHAIN STATUS FROM: "+h+" "+this.index.hash[h]);
...
Blockchain.prototype.updateLongestChain = function updateLongestChain(newblock, shared_ancestor_index_pos, nchain_len, lchain_len, forceAdd) {
  ...
  if (shared_ancestor_index_pos > -1) {
    ...
    for (var h = shared_ancestor_index_pos; h < this.index.lc.length; h++) {
      this.index.lc[h] = 0;
console.log("SETTING BLOCK TO ZERO: "+this.index.block_id[h] + " -- " + this.index.hash[h]);
      this.app.storage.saveLongestChainStatus(this.index.hash[h], this.index.block_id[h], 0);
...
console.log("SETTING BLOCK TO ONE: "+this.index.block_id[j] + " -- " + this.index.hash[j]);
...
//console.log("\n\n\nsetting newchain index to this: \n\n\n");
...
Blockchain.prototype.deIndexAndPurge = function deIndexAndPurge(blk) {
  ...
  var pos        = this.returnIndexPositionOfBlock(blk);
  var prevblk    = this.returnBlockByHash(blk.block.prevhash);
  var prevblkpos = this.returnIndexPositionOfBlock(prevblk);
...
Blockchain.prototype.addBlock = function addBlock(blk, relay_on_validate=1) {
  ...
  //////////////////////////////
console.log("RECLAIMED FUNDS VALIDATED (pre): "+blk.block.reclaimed);

  blk.validateReclaimedFunds(function(validated_or_not) {

console.log("RECLAIMED FUNDS VALIDATED: "+validated_or_not);
...
//console.log("RETURNING LONGEST BLOCK: "+this.index.hash[this.longestChain]);
//console.log("LC: "+this.longestChain);
//console.log("BL: "+this.blocks.length);
  if (this.blocks.length == 0) { return null; }
  for (var i = this.blocks.length-1; i >= 0; i--) {
    if (this.blocks[i].hash() == this.index.hash[this.longestChain]) {
//console.log("FOUND LONGEST CHAIN: "+i);
