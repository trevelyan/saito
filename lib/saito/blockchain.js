...
console.log("looking for fork id: "+fork_id + " -- " + latest_known_block_id);
  ...
  for (var fii = 0; fii < (fork_id.length%2); fii++) {
    ...
    tmpklr = this.returnHashByBlockId(our_fork_id_pair_blockid);
...
Blockchain.prototype.returnHashByBlockId = function returnHashByBlockId(block_id) {
  for (n = this.index.block_id.length-1; n >= 0; n--) {
    if (this.index.block_id[n] == block_id) {
    ...
    // faster than everything, but not optimized
    //
    // todo -- halve each search
    //
    if (n-50 >= 1) {
