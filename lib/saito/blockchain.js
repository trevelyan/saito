...
Blockchain.prototype.debug = function debug() {
  ...
  for (mb = 0; mb < this.blocks.length; mb++) {
    ...
    if (longestchainhash == this.blocks[mb].hash()) {
      console.log("***** " + this.blocks[mb].block.id + " -- " + this.blocks[mb].hash() + "  ----->  " + this.blocks[mb].block.prevhash);
    } else {
      console.log(this.blocks[mb].block.id + " -- " + this.blocks[mb].hash() +  "  ----->  " + this.blocks[mb].block.prevhash);
    }
  }
  ...
  for (i = 0; i < this.index.hash.length; i++) {
    console.log(this.index.lc[i] + " ----> " + this.index.block_id[i] + " || " + this.index.hash[i] + " || " + this.index.prevhash[i]);
...
Blockchain.prototype.indexAndStore = function indexAndStore(newblock, forceAdd="no") {
  ...
  console.log("Adding "+newblock.hash());
  ...
  if (prevhash == this.index.hash[this.longestChain]) {
    ...
    //
    //
    if (this.returnLatestBlock().returnBurnFee() > newblock.returnBurnFee()) {
...
Blockchain.prototype.updateForkId = function updateForkId(blk) {
  ...
  for (i = 0, stop = 0; stop == 0, i < this.genesis_period;) {
    ...
    blockhash = "asgasdfasdfasdfasdfasdfasdfasfd";

    fork_id += blockhash.substring(0,2);
    ...
    if (i > this.genesis_period || i == 500000) { stop = 1; }
...
console.log("IMPORTING blk: "+nb.returnId());
...
Blockchain.prototype.returnLongestChain = function returnLongestChain(chainlength=10) {
  ...
  finished = 0;
  for (i = chainarray.length-1; i >= 0 && finished == 0; i--) {
    tmpblk = this.returnBlockByHash(this.index.hash[chainarray[i]]);
    if (tmpblk != null) {
...
Blockchain.prototype.returnBlockHashById = function returnBlockHashById(spos=0) {
  if (this.index.hash.length == 0) { return null; }
  start_pos = this.index.hash.length-1;
  if (spos != 0) { start_pos = spos; }
