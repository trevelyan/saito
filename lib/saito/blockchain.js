...
Blockchain.prototype.addBlockToBlockchain = function addBlockToBlockchain(newblock, forceAdd="no") {
  ...
  if (i_am_the_longest_chain == 1) {
    ...
    // validate slips //
    ////////////////////
    this.app.storage.validateLongestChain(newblock, shared_ancestor_index_pos, new_block_idxs, new_block_hashes, old_block_hashes, old_block_idxs);

  } else {

    // JUMP RIGHT TO THE SAVING PART
...
/****
  ...
  //////////////////////////////////////////////////////////
  //
  // it is possible for us to run into validation errors
  // if we try to validate a block that has not been 
  // properly saved to dis
  ...
  if (	blockchain_self.app.storage.saving_blocks == 1 || blockchain_self.app.storage.saving_slips == 1 || blockchain_self.app.storage.spending_slips == 1) {
    blockchain_self.block_saving_timer = setInterval(function() {
      blockchain_self.addBlockToBlockchainPartTwo(newblock, pos, validate_transactions, hash, old_longestChain, forceAdd,  rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, i_am_the_longest_chain);
...
//
// This function follows the first. It is called after a timer expires that
// confirms that newblock was saved successfully to disk. This avoids 
// potential issues with slip validation that can happen if we look locally
// for a record of the slips we need to validate and cannot find them for 
// some reason.
//
Blockchain.prototype.addBlockToBlockchainPartTwo = function addBlockToBlockchainPartTwo(newblock, pos, validate_transactions, hash, old_longestChain, forceAdd,  rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, i_am_the_longest_chain) {
  ...
  // hashmap //
  /////////////
  this.lc_hashmap[hash] = i_am_the_longest_chain;
  ...
  if (validate_transactions != -1) {
    ...
    // slip validation is handled by a separate function. This will
    // check that all of the slips are valid for the ENTIRE newchain
    // which may just consist of this one block, but may also involve
    // a chain reorganization.
    //
    // because this function involves (or may involve) opening files
    // on-disk, we need to wait until our storage class calls the 
    // following functions through callbacks:
    //
    //   addBlockToBlockChainSuccess
    //   addBlockToBlockChainFailure
    //
    // The first callback will trigger the remainder of the block
    // processing, such as adding the money to the wallet and invoking
    // callbacks, etc. while Failure will ensure the chain is our 
    // former longest chain and reset us for the next block to add
    //
    this.validateLongestChain(newblock, rewrite_shared_ancestor_index_pos, rewrite_nchain_len, rewrite_lchain_len, forceAdd, old_longestChain, 2, validate_transactions);

  } else {

    this.addBlockToBlockchainSuccess(this.app, null, newblock, forceAdd);
...
// us.
//
Blockchain.prototype.addBlockToBlockchainSuccess = function addBlockToBlockchainSuccess(app, newblock, forceAdd) {

  // because triggered through callbacks
  var blockchain_self = app.blockchain;
...
Blockchain.prototype.addBlockToBlockchainFailure = function addBlockToBlockchainFailure(blk, old_lc, lchain_len) {
/***********
