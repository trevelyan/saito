...
function Blockchain(app) {
  ...
  this.money_supply            = 2000;  // zero-inflation target
  this.genesis_period          = 100;     // all money released in this frame
...
Blockchain.prototype.debugHTML = function debugHTML() {
  ...
  for (mb = 0; mb < this.blocks.length; mb++) {
    ...
    longestchainhash = this.index.hash[this.longestChain];
    if (longestchainhash == this.blocks[mb].hash()) {
      html += '<td>*****</td><td>'+this.blocks[mb].block.id+'</td><td>'+this.blocks[mb].hash()+'</td><td>'+this.blocks[mb].block.prevhash+'</td>';
    } else {
      html += '<td></td><td>'+this.blocks[mb].block.id+'</td><td>'+this.blocks[mb].hash()+'</td><td>'+this.blocks[mb].block.prevhash+'</td>';
    }
    html += '</tr>';
