...
Network.prototype.propagateTransaction = function propagateTransaction(tx, outboundMessage="transaction", mycallback=null) {
  ...
  // not perfect, but we check to see if we are capable of creating a block
  // before we propagate the transaction to the rest of the network, in 
  // order to avoid sending a transaction and a block simultaneously.
  if (this.app.blockchain.mempool.containsTransaction(tx) != 1) {
    ...
    this.app.blockchain.mempool.bundleLoop();
    if (this.app.blockchain.mempool.returnBundlingFeesNeeded() <= 0) { return null; }
  }
