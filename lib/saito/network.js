...
Network.prototype.propagateTransaction = function propagateTransaction(tx, outboundMessage="transaction") {
  ...
  for (networki = 0; networki < this.peers.length; networki++) {

console.log("CHECKING TO SEE IF WE ARE IN THE TRANSACTION PATH");
console.log("TP: "+ this.peers[networki].inTransactionPath(tx));
...
console.log("OUTBOUND TRANSACTION: ");
console.log("SENDING: ");
console.log(tmptx.transaction);
    ...
    // if the peer isn't on the transaction path but the transaction
    // comes from us then we want to propagate it too
