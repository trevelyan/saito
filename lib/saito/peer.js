...
Peer.prototype.connect = function connect(remote=0) {
  ...
  if (remote == 0) {
    ...
    if (this.app.options.blockchain.lastblock != "") {
      message.data.lastblock    = this.app.options.blockchain.lastblock;
    }
...
console.log("REQUESTING DATA FROM : "+message.data.lastblock);
