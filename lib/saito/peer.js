...
Peer.prototype.addSocketEvents = function addSocketEvents() {
  ...
  this.socket.on('connect', function(){
    ...
    // log connection
    if (peer_self.app.BROWSER == 0) {
      var address = peer_self.socket.request.connection.remoteAddress;
      peer_self.app.storage.writeToLog(new Date().getTime() + " -- connection: " + address);
    }
  ...
  this.socket.on('disconnect', function(){
    ...
    if (peer_self.app.BROWSER == 0) {
      var address = peer_self.socket.request.connection.remoteAddress;
      peer_self.app.storage.writeToLog(new Date().getTime() + " -- disconnect: " + address);
    }
  ...
  this.socket.on('request', function (data, mycallback=null) {
    ...
    if (message.request == "connect") {
      ...
      if (peer_self.app.BROWSER == 0) {
        var address = peer_self.socket.request.connection.remoteAddress;
        peer_self.app.storage.writeToLog(new Date().getTime() + " -- connect: " + address + " " + peer_self.peer.publickey);
      }
