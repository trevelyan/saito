...
console.log("PAYMENT REQUESTED: "+peer_self.peer.publickey);

      // try to send a payment just to see
      if (peer_self.peer.publickey != "") {
console.log("MY BALANCE: "+peer_self.app.wallet.returnBalance());
...
console.log("THIS STUFF RETURNED: ");
console.log(message.data.dns);
...
Peer.prototype.connect = function connect(remote=0) {
  ...
  if (remote == 0) {
    ...
    // refuse to cause problems out of principl
    if (this.peer.host == "localhost" && this.app.BROWSERIFY == 0) { return; }
