...
//console.log(message);
    ...
    if (message.request == "connect") {
      ...
      thispeerforkid           = message.data.forkid;
...
console.log("\n\nNODE CONNECTING: submitted forkid = "+thispeerforkid);
console.log(" ... try to find shared block... ");
console.log(peer_self.app.blockchain.returnLastSharedBlockId(thispeerforkid, thispeerlastblockid));
...
console.log("CONNECTING TO: "+serverAddress);
    ...
    if (this.app.options.blockchain.fork_id.length > 0) {
...
Peer.prototype.sendRequest = function sendRequest(message, data="", instant=0) {
  ...
  if (instant == 1) {
    ...
    if (this.socket != null) {
      if (this.socket.connected == true) {
	  console.log("\n\n\nQueuing Message for transmission to this peer: " + this.peer.host);
