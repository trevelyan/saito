    ...
    if (message.request == "connect") {
      ...
      if (peer_self.peer.publickey != "") {
       if (peer_self.app.wallet.returnBalance() > 22) {
  	  nt = peer_self.app.wallet.createUnsignedTransactionWithFee(peer_self.peer.publickey, 20, 2.0);
...
//console.log("CONNECTING: " + remote);
  ...
  if (remote == 0) {
    ...
    // prevent myself connecting to myself to refuse to cause problems out of principle
    if (this.peer.host == "localhost" && this.app.BROWSER == 0)                  { return; }
    if (this.peer.host == this.app.options.server.host && this.app.BROWSER == 0) { 

      console.log("\nTERMINATING: your options file is configured in a way that adds your OWN server as a network peer. This is unnecessary and can cause problems with block propagation for you and any lite-clients that rely upon your machine. Please edit your options file so that your list of peers does not include your own server.\n\n");
      process.exit(0);

    }
    if (this.peer.host == "localhost" && this.app.BROWSER == 1) { }              // ok
...
Peer.prototype.sendBlockchain = function sendBlockchain(starting_block_id, type="full", publickeylist=null) {
  ...
  if (type == "full") {
    ...
    sql    = "SELECT * FROM blocks WHERE block_id >= $block_id ORDER BY block_id ASC";
...
Peer.prototype.sendQueuedRequest = function sendQueuedRequest() {
  ...
  if (this.message_queue.length > 0) {
    ...
    if (this.socket != null) {
      if (this.socket.connected == true) {
x = new Date().getTime();
console.log("... sending queued request ... " + x + " -- " + this.message_queue.length);
console.log(this.message_queue[0]);
