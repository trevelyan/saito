...
Peer.prototype.addSocketEvents = function addSocketEvents() {
  ...
  this.socket.on('connect', function(){
    ...
    if (peer_self.contact == 1) {
      ...
      // we initiated contact -- re-connect
...
console.log("\n\n\n\n CONNECTION REQUEST WITH PUBLIC KEY: "+message.data.publickey);
...
console.log("SENDING UNSIGNED TX WITH FEE");
console.log(message);
...
console.log("HERE: "+peer_self.peer.synctype);
...
Peer.prototype.connect = function connect(remote=0) {
  ...
  if (remote == 0) {
    ...
    //  msg = peer_self.returnBlockchainRequest(peer_self.peer.synctype, peer_self.app.blockchain.returnLatestBlockId()+1);
    //  socket.emit('request',JSON.stringify(msg));
    //
    //}, this.ping_speed);
...
console.log("almost sending message to "+this.peer.host);
  ...
  if (instant == 1) {
    ...
    if (this.socket != null) {
      if (this.socket.connected == true) {
console.log("sending message to "+this.peer.host);
