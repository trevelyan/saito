...
Peer.prototype.returnConnect = function returnConnect(request_tokens = 0) {
	...
	var message = {};
	    ...
	    message.data.keylist          = this.app.keys.returnWatchedPublicKeys().push(this.app.wallet.returnPublicKey());
...
Peer.prototype.returnBlockchainRequest = function returnBlockchainRequest(type="full", block_id=0) {
  ...
  response.data.keylist                  = this.app.keys.returnWatchedPublicKeys().push(this.app.wallet.returnPublicKey());
  return response;
}
