...
function Peer(app, peerjson="") {
  ...
  this.ping_speed            = 120000; // ping for new blocks every 2 minutes
...
Peer.prototype.addSocketEvents = function addSocketEvents() {
  ...
  this.socket.on('connect', function(){
	...
	// we should reply when they connect below

      // they initiated contact
      //var message = peer_self.returnConnectResponse();
  ...
  this.socket.on('request', function (data, mycallback=null) {
    ...
    if (message.request == "connect") {
      ...
      if (server_guesstimated_last_shared_id < thispeerlastblockid) {
	if (peer_self.peer.synctype == "full") {
	  thispeerlastblockid = server_guesstimated_last_shared_id; 
	}
      }
      ...
      if (peer_self.peer.synctype == "lite") {
        if (thispeerlastblockid === "") {
          ...
          if (thispeerlastblockid == "") { thispeerlastblockid = 0; }
      ...
      // we send the response elsewhere (we should, we think!)
    ...
    if (message.request == "connect-reply") {
      ...
      //var updateRemoteServer = 0;

      // doublecheck -- is the other server behind us in its block?
      //
      // if they seem to be, we send them what is missing if we
      // are a full-node connection
      var mylastblockid       = peer_self.app.blockchain.returnLatestBlockId();
      var their_last_block_id = message.data.current_block_id;
      var their_fork_id       = message.data.current_fork_id;

      if (their_last_block_id < mylastblockid) {
        if (peer_self.peer.synctype == "lite") {
          peer_self.sendBlockchain(their_last_block_id, synctype, pklist);
        } else {
      ...
      if (peer_self.peer.publickey == "") { updateRemoteServer = 1; }
      if (peer_self.peer.synctype == "lite" && message.data.synctype == "full") { 
	//updateRemoteServer = 1;
	peer_self.peer.synctype = "lite"; 
      }

      //if (updateRemoteServer == 1) {
        //var response = peer_self.returnConnectResponse();
        //peer_self.socket.emit('request',JSON.stringify(response));
      //}
...
// (servers can ignore this). We are set to request credit when our balance
// falls below 10 tokens.
//
...
Peer.prototype.sendRequest = function sendRequest(message, data="", instant=0) {
  ...
  // lite-clients
  if (message == "block" && this.peer.synctype == "lite") {

    var blk = new saito.block(this.app, userMessage.data);
    var is_important_block = 1;
    ...
    if (this.peer.watchedkeylist.length == 0) {
      if (! blk.containsTransactionFor(this.peer.publickey)) {
        is_important_block = 0;
      } 
    } else {
      is_important_block = 0;
      for (var itib = 0; itib < this.peer.watchedkeylist.length && is_important_block == 0; itib++) {
        if (blk.containsTransactionFor(this.peer.watchedkeylist[itib])) {
          is_important_block = 1; 
        } 
      } 
      if (is_important_block == 0) {
...
Peer.prototype.sendRequestWithCallback = function sendRequestWithCallback(message, data="", mycallback) {
  ...
  if (this.socket != null) {
    if (this.socket.connected == true) {
console.log("EMITTING THE REQUEST NOW");
