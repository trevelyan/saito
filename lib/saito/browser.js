...
Browser.prototype.monitorBlockchainSyncing = function monitorBlockchainSyncing(remote_blkid, remote_genesis_blkid) {
  ...
  // if our client.json value is the same as the 
  // blockId the server tells us is the newest we 
  // are a new client and should just skip the entire
  // monitoring effort
...
console.log("\n\n\n\nSET INTERVAL TIMER: "+browser_self.monitor_timer);

    // count through our indexed blocks to see if our missing blocks 
    // have been added.
    found = 1;
    ...
    console.log("Percent Synced: "+percent_downloaded);
    console.log(local_block_id + " -- " + remote_block_id);


console.log("HERE WE ARE PERCENT DOWNLOADED! "+percent_downloaded);
    if (percent_downloaded == 100) {
console.log("HERE WE ARE PERCENT DOWNLOADED! "+percent_downloaded);
      // ensure we update our options file
      browser_self.app.storage.saveOptions();
console.log("stopping the interval that updates our monitor");
console.log("\n\n\n\nCUT INTERVAL TIMER: "+browser_self.monitor_timer);
