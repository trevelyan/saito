...
/****
    ...
    for (i = 0; i < this.app.options["Messages"].length; i++) {
      ...
      if (tx.message.module != "") {
        ...
        browser_self.app.modules.attachMessageToBrowser(tx, txmod, browser_self);
      }
    }
****/
...
Browser.prototype.attachEvents = function attachEvents() {
      ...
      $('#lightbox_compose_module_select').on('change', function() {
        ...
        self.app.modules.updateBrowser(modsel);
      });
        ...
        $.fancybox({
          ...
          afterShow : function(){
           ...
           $('#send').on('click', function() {
             ...
             newtx = self.app.wallet.createUnsignedTransaction(to, amount);
             modsel = $('#lightbox_compose_module_select').val();
             newtx = self.app.modules.formatMessage(newtx, modsel);
             newtx = self.app.wallet.signTransaction(newtx);

             self.app.network.propagateTransaction(newtx);
      ...
      $('.message').on('click', function() {
        ...
        $('#lightbox_message_from_address').text( self.app.wallet.returnPublicKey() );
        ...
        self.app.modules.displayMessage(message_id, module);
