...
';
  ...
  $('.post_controls_comment').on('click', function() {
   ...
   $('.post_comments_button').on('click', function() {
     ...
     newtx = app.wallet.signTransaction(newtx);
console.log("PROPAGATING THIS TX");
console.log(newtx);
     app.network.propagateTransaction(newtx);
console.log("DONE PROPAGATING TX");
...
console.log("ABOUT TO SAVE: "+am_i_following+" - "+is_for_me+" - "+is_from_me);
    ...
    if (am_i_following == 1) {
console.log("SAVING BECAUSE I AM FOLLOWING:");
      app.archives.saveMessage(tx);
    } else {

      // SAVING MY POSTS
      if (tx.transaction.msg.type == "post" && (is_from_me == 1 || is_for_me == 1)) {
console.log("SAVING BECAUSE THIS IS MY POST:");
        app.archives.saveMessage(tx);
      }


      // OR SAVE MY COMMENT + parent post
      if (tx.transaction.msg.type == "comment" && (is_from_me == 1 || is_for_me == 1)) {

console.log("SAVING AS RELEVANT COMMENT");
        if (is_for_me == 0) {

 	  // save parent post -- I dont have this saved


	  parent_tx = myfacebook.generateTransactionByPostId(tx.transaction.msg.post_id);

	  // comment is FOR someone else, but it is FROM me
console.log("RETURNED FROM GENERATED TX:");
console.log(parent_tx);
...
console.log("submitted post: "+author);
...
Facebook.prototype.generateTransactionByPostId = function generateTransactionByPostId(post_id, childtx=null) {
