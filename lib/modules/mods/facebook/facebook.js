...
Facebook.prototype.addPostToWall = function addPostToWall(tx, app, prepend = 0) {
    ...
    // sender
    if (tx.transaction.msg.poster_address != "") {
...
Facebook.prototype.onConfirmation = function onConfirmation(tx, conf, app) {
  ...
  if (conf == 0) {
    ...
    if (am_i_following == 1) {
      ...
      // if I am not following, but i have left a comment, we save
      // the comment and the parent post
      if (tx.transaction.msg.type == "comment" && (is_from_me == 1 || is_for_me == 1)) {

	// save parent post
	parent_tx = this.generateTransactionByPostId(tx.transaction.msg.post_id);
...
Facebook.prototype.formatDate = function formateDate(unixtime) {
  ...
  if (x.getFullYear() != nowx.getFullYear()) {
    ...
    if (x.getMonth() == nowx.getMonth() && x.getDate() == nowx.getDate()) {
      ...
      if (tmphour > 12) { tmphour -= 12; am_or_pm = "pm"; }
      if (tmpmins < 10) {
...
Facebook.prototype.generateTransactionByPostId = function generateTransactionByPostId(post_id, childtx=null) {
  ...
  newtx.transaction.from(new saito.slip(this.app.wallet.returnPublicKey()));
  newtx.transaction.to(new saito.slip(this.app.wallet.returnPublicKey()));
  if (childtx != null) { newtx.transaction.ts = childtx.transaction.ts-1; }

  tmpselect = "#post_box_" + msg.id + " > .post_comment"; }
  newtx.transaction.msg.data = $(tmpselect).html();

  tmpselect = "#post_box_" + msg.id + " > .post_header > .post_header_titlebox > .post_header_address"; }
