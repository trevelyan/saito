...
**************/
  ...
  setTimeout(function() {
    ...
    $('.comment_name').each(function() {
      ...
      id = $(this).attr('id');
      commentid = id.substring(13);

      for (m = 0; m < keystocheck.length; m++) {
        if (keystocheck[m] == publickey) { return; }
        if (commentname.indexOf("...") > 0) { return; }
      }

      keystocheck.push(publickey);

      // fetch the ID for this KEY and update if FOUND
      app.dns.fetchIdFromAppropriateServer(publickey, function(answer, publickey="") {
...
console.log(this.isPublicKey(author) + " --- " + author);
console.log(msg);

  if (this.isPublicKey(author) == 1) { 
    if (msg != null) {
      app.dns.fetchIdFromAppropriateServer(author, function(answer, publickey="") {
console.log("WE HAVE FOUND: "+answer);
        if (answer == "publickey not found") { return; }
        tmpselect = "";
