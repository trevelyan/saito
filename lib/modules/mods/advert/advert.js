...
function Advert(app) {
  ...
  this.advert_img_dir  = "/Users/admin/Programming/saito/lib/modules/mods/advert/web/cache/";
...
Advert.prototype.initializeHTML = function initializeHTML() {
    ...
    $('#advert-728-90').html('<img style="width:728px;height:90px" src="/advert/'+this.app.wallet.returnPublicKey()+'" />');
    $('#advert-336-280').html('<img style="width:336px;height:280px" src="/advert/'+this.app.wallet.returnPublicKey()+'" />');
    $('#advert-320-100').html('<img style="width:320px;height:100px" src="/advert/'+this.app.wallet.returnPublicKey()+'" />');
    $('#advert-300-250').html('<img style="width:300px;height:250px" src="/advert/'+this.app.wallet.returnPublicKey()+'" />');
    $('#advert-250-250').html('<img style="width:250px;height:250px" src="/advert/'+this.app.wallet.returnPublicKey()+'" />');
    $('#advert-120-90').html('<img style="width:120px;height:90px" src="/advert/'+this.app.wallet.returnPublicKey()+'" />');
...
Advert.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.post('/advert/upload/:publickey', function (req, res) {
    ...
    var uploadUrl  = req.files.uploadUrl;
    ...
    advert_self.app.storage.db.run(sql, params, function(err, row) {
      ...
      sampleFile.mv(newFileName, function(err) {
        ...
        res.send('File uploaded: '+newFileName);
      });
  ...
  expressapp.get('/advert/:publickey', function (req, res) {
    ...
    advert_self.app.storage.queryDatabase(psql, pparams, function(perr, prow) {
	  ...
	  if (prow2 != null && prow2.length > 0) {
            res.sendFile(__dirname + '/web/img/' + prow2[0].id + '.jpg');
            return;
	  } else {
...
Advert.prototype.handlePeerRequest = function handlePeerRequest(app, message, peer, mycallback) {
    ...
    if (message.request === "advert load response") {
      ...
      var toInsert = '<div><img src="/advert/cache/'+advert_id+'.jpg" style="height:150px;width:125px;" /><a href="'+advert_link+'">'+advert_link+'</a></div>';
