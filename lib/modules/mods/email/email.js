...
Email.prototype.formatAuthor = function formatAuthor(author, app) {
  ...
  if (x != null) {
    if (x.identifiers.length > 0) {
      return x.identifiers[0];
    }
  }

  if (x == this.app.wallet.returnPublicKey()) {
    if (this.app.wallet.returnIdentifier() == "") {
      return "me";
    }
  }
  ...
  if (this.isPublicKey(author) == 1) {
    app.dns.fetchIdFromAppropriateServer(author, function(answer) {
      ...
      $('.from').each(function() {
        pkey = $(this).text();
        if (pkey == dns_response.publickey) {
          $(this).text(dns_response.identifier);
        }
      });
