...
Encrypt.prototype.displayEmailForm = function displayEmailForm(app) {
  ...
  element_to_edit.html('<div style="min-height:100px;padding-top:120px;padding:40px;font-size:1.6em;line-height:2.4em;">Click send to initiate a Diffie-Hellman key-exchange over the Saito blockchain. Once your recipient approves your request, you will be notified and all subsequent email with them will be automatically encrypted.</div>');
}
...
console.log("Encrypt conf 1 - " + conf);
...
console.log("Encrypt conf 2 "  + receiver + " -- " + app.wallet.returnPublicKey());
...
console.log("Encrypt conf 3");

        app.modules.returnModule("Email").attachMessage(msg, app);
console.log("Encrypt conf 4");
	...
	alice            = app.crypt.createDiffieHellman(alice_publickey, alice_privatekey);
        ...
        app.aes.updateCryptoByPublicKey(sender, alice_publickey.toString("hex"), alice_privatekey.toString("hex"), alice_secret.toString("hex"));
        app.aes.saveAes();
...
console.log("ENCR AUTH LINK CLICKED");
      ...
      utx = app.wallet.createUnsignedTransactionWithFee(remote_address, 1.5, 2.0);
      if (utx == null) { return; }
      ...
      app.aes.updateCryptoByPublicKey(thistx.transaction.from[0].add, bob_publickey, bob_privatekey, bob_secret.toString("hex"));
      app.aes.saveAes();
