...
function Registry(app) {
  ...
  this.publickey       = "29otZ7bFUnffwdi5Hx9YMbm3sPA9CAqwQPkxgAhpaefmz";
    ...
    if (registry_self.app.wallet.returnPublicKey() != registry_self.publickey) {
	  ...
	  if (body != null) {
		...
		if (signer != registry_self.publickey) {} else {
	     	  ...
	     	  var msgtosign   = identifier + address + unixtime;
	    	  var msgverifies = app.crypt.verifyMessage(msgtosign, sig, signer);
  ...
  if (conf == 0) {
    ...
    if (tx.transaction.msg != null) {
      ...
      app.storage.queryDatabase(tmsql, params, function(err, row) {
	    ...
	    var sqlwrite = full_identifier + "\t" + tx.transaction.from[0].add + "\t" + tx.transaction.ts + "\t" + registrysig + "\t" + app.wallet.returnPublicKey() + "\n";
