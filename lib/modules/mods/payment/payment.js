...
<p></p> \
Sending messages across the Saito network requires tokens. You can purchase tokens peer-to-peer from other users, or directly <a href="/payment?saito_buyer='+app.wallet.returnPublicKey()+'&saito_identifier='+app.wallet.returnIdentifier()+'">from our server</a>. \
</div> \
<style> \
';
...
Payment.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/payment/', function (req, res) {
    ...
    var saito_buyer     = "";
    if (req.query.saito_buyer != null) { saito_buyer = req.query.saito_buyer; }

    res.setHeader('Content-type', 'text/html');
    res.charset = 'UTF-8';
    res.write(payment_self.returnHTML(saito_buyer));
    res.end();
    return;
  ...
  expressapp.get('/payment/ethereum', function (req, res) {
    ...
    if (req.query.saito_buyer == null || req.query.ethereum_buyer == null || req.query.purchase_ts == null) {
      res.setHeader('Content-type', 'text/html');
      res.charset = 'UTF-8';
    ...
    var purchase_ts     = req.query.purchase_ts;
    ...
    if (saito_buyer == null || ethereum_buyer == null) {
      ...
      var sql    = "INSERT INTO mod_payments (publickey, ethereum, amount, unixtime, tokens_sent) VALUES ($saito_buyer, $ethereum_buyer, $amount, $unixtime, $tokens_sent)";
      var params = { $saito_buyer : saito_buyer, $ethereum_buyer: ethereum_buyer, $amount : amount, $unixtime : purchase_ts , $tokens_sent : 0 };
      app.storage.execBlockchain(sql, params, function(err, row) {

        res.setHeader('Content-type', 'text/html');
        res.charset = 'UTF-8';
        res.write(payment_self.returnEthereumHTML(saito_buyer, ethereum_buyer, ethereum_seller, purchase_ts));
        res.end();
	...
	// send an email
        ...
        newtx.transaction.msg.data  = 'You have requested the purchase of '+payment_self.saito_amount+' tokens. This requires your  \
					...
					<p></p> 										    \
					'+payment_self.ethereum_seller_address+'						    \
					<p></p>											    \
					Once this payment has received three confirmations, <a href="'+payment_self.server_address+'ethereum/confirm?purchase_ts='+purchase_ts+'&saito_buyer='+saito_buyer+'&ethereum_buyer='+ethereum_buyer+'">click here to complete your purchase</a> \
					';
...
Payment.prototype.returnHTML = function returnHTML(saito_buyer="") {
...
\
  ...
  if (this.app.wallet.returnBalance() <= (this.saito_amount + this.saito_fee)) {
    ...
    var purchase_ts = new Date().getTime();

    pagehtml += ' \
      <h2>'+payment_self.saito_amount+' Saito Tokens:</h2> \
      \
      <p></p> \
      We need your Saito address and the Ethereum (ETH) address that will make payment. \
      \
      <p></p> \
      \
      <form action="/payment/ethereum" method="get"> \
      <table><tr><td class="table_label"> \
        <label for="saito_buyer">Saito Address:</label> \
      </td><td class="table_input"> \
        <input type="text" name="saito_buyer" value="' + saito_buyer +'" /> \
      </td></tr><tr><td class="table_label"> \
        <label for="ethereum_buyer">Ethereum Address:</label> \
      </td><td class="table_input"> \
        <input type="text" name="ethereum_buyer" /> \
        <input type="hidden" name="purchase_ts" value="'+purchase_ts+'" /> \
      </td></tr><tr><td></td><td> \
...
Payment.prototype.returnEthereumHTML = function returnEthereumHTML(saito_buyer, ethereum_buyer, ethereum_seller, purchase_ts) {
...
\
    <div class="main"> \
      ...
      <script type="text/javascript"> \
        new QRCode(document.getElementById("qrcode"), "'+ethereum_seller+'"); \
      </script> \
      <br /> \
      '+ethereum_seller+' \
      <p></p> \
      <b>Once your payment has <b>three confirmations</b>, <a href="/payment/ethereum/confirm?purchase_ts='+purchase_ts+'&saito_buyer='+saito_buyer+'&ethereum_buyer='+ethereum_buyer+'">click here to complete your transaction</a>. \
    </div> \
\
</body> \
</html>';
