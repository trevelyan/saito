...
function Payment(app) {
  ...
  this.ethereum_timer_speed = 1000;
...
Payment.prototype.initialize = function initialize() {
  ...
  this.ethereum_timer = setInterval(function() {
    ...
    var sql = "SELECT * FROM mod_payments WHERE payment_received = 0 AND unixtime > $unixtime_limit AND waiting_for_payment = 1";
    var params = {
      $unixtime_limit : unixtime_limit
    }
    payment_self.app.storage.queryDatabaseArray(sql, params, function(err, rows) {
      if (rows != null) {
        for (var i = 0; i < rows.length; i++) {
          var payment_to_check = rows[i];
console.log("WE ARE SUPPOSED TO CHECK THE STATUS OF THIS PAYMENT: ");
...
Payment.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/payment', function (req, res) {
    ...
    var sql    = "INSERT INTO mod_payments (publickey, ethereum_pubkey, ethereum_privkey, amount, unixtime, last_checked, waiting_for_payment, payment_received, tokens_sent) VALUES ($saito_buyer, $ethereum_pubkey, $ethereum_privkey, $amount, $unixtime, $last_checked, $waiting_for_payment, $payment_received, $tokens_sent)";
