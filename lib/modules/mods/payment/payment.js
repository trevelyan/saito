...
console.log("ethereum timer loop...");
    ...
    payment_self.app.storage.queryDatabaseArray(sql, params, function(err, rows) {
	  ...
	  payment_self.checkEthereumApi(payment_self.app, rows[i].id, rows[i].ethereum_pubkey);
...
<p></p> \
<a href="/payment?saito_buyer='+app.wallet.returnPublicKey()+'">Click here to purchase SAITO tokens from our server</a>. \
</div> \
<style> \
';
...
Payment.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/payment', function (req, res) {
    ...
    var sql    = "INSERT INTO mod_payments (publickey, ethereum_pubkey, ethereum_privkey, amount, unixtime, last_checked, times_checked, waiting_for_payment, payment_received, tokens_sent) VALUES ($saito_buyer, $ethereum_pubkey, $ethereum_privkey, $amount, $unixtime, $last_checked, $times_checked, $waiting_for_payment, $payment_received, $tokens_sent)";
    ...
    app.storage.execBlockchain(sql, params, function(err, row) {
      ...
      newtx.transaction.msg.title  = "Saito Token Purchase";
      newtx.transaction.msg.data   = 'You have requested the purchase of '+payment_self.saito_amount+' tokens. This requires your  \
				     ...
				     '+ethereum_pubkey+'						    \
				     <p></p>											    \
			             If you have not received your tokens after three confirmations, please <a href="'+payment_self.server_address+'ethereum/confirm?unixtime='+unixtime+'&saito_buyer='+saito_buyer+'&ethereum_pubkey='+ethereum_pubkey+'">click here to complete your purchase</a> \
					';
  ...
  expressapp.get('/payment/ethereum/confirm', function (req, res) {
    ...
    var sql = "UPDATE mod_payments SET waiting_for_payment = 1 WHERE unixtime = $unixtime AND ethereum_pubkey = $ethereum_pubkey";
    var params = {
      $unixtime : unixtime,
      $ethereum_pubkey : ethereum_pubkey
    };
    payment_self.app.storage.execDatabase(sql, params, function(){});
...
console.log(json);
      ...
      if (tx_found == 1) {
        ...
        var params2 = { $id : mod_payments_api };
    	...
    	if (newtx == null) {
          ...
          var params3 = { $id : mod_payments_api };
...
\
  ...
  if (this.app.wallet.returnBalance() <= (this.saito_amount + this.saito_fee)) {
    ...
    pagehtml += ' \
      ...
      '+ethereum_pubkey+' \
      <p></p> \
      <b>Once you have made payment, please <a href="/payment/ethereum/confirm?unixtime='+unixtime+'&saito_buyer='+saito_buyer+'&ethereum_pubkey='+ethereum_pubkey+'">click here to complete your transaction</a>. \
    </div> \
    ';
