  ...
  expressapp.get('/faucet/success', function (req, res) {
    ...
    faucet_self.app.storage.queryDatabase(sql, params, function(err, rows) {
      ...
      if (can_send == 0) {
        ...
        // send an email
        newtx = faucet_self.app.wallet.createUnsignedTransactionWithFee(saito_address, 15.0, 2.0);
        if (newtx == null) { return; }
        newtx.transaction.msg.module = "Email";
        newtx.transaction.msg.title  = "Saito Faucet - Transaction Receipt";
        newtx.transaction.msg.data   = 'You have received 15 tokens from our Saito faucet.';
...
Faucet.prototype.initializeHTML = function initializeHTML(app) {
    ...
    $('#saito_address_show').text(app.wallet.returnPublicKey());
}
