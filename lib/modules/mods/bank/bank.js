...
console.log("Formatting Email Transaction for the Bank Module");
...
console.log("Enter withdrawal?");
  if (bank_req == "withdrawal") {
console.log("YES! Enter conf == 0?");
  if (conf == 0) {

    var withdrawal_address = tx.transaction.from[0].add;

console.log("WITHDRAW TO: "+withdrawal_address);
...
console.log("Does TX validate?");
console.log(newtx.validate(bank_self.app));
          bank_self.app.blockchain.mempool.addTransaction(newtx);
          bank_self.app.network.propagateTransaction(newtx);
