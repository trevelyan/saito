...
console.log(filename);
	    ...
	    app.storage.openBlock(filename, function(app, data) {
              res.setHeader('Content-type', 'text/html');
              res.charset = 'UTF-8';
console.log("Here 2");
...
Explorer.prototype.listBlocks = function listBlocks() {
  ...
  for (var mb = explorer_self.app.blockchain.blocks.length-1; mb >= 0 && mb > explorer_self.app.blockchain.blocks.length-200; mb--) {
    ...
    if (longestchainhash == explorer_self.app.blockchain.blocks[mb].hash()) {
      html += '<td>*</td><td><a href="/explorer/block?hash='+explorer_self.app.blockchain.blocks[mb].hash('hex')+'">'+explorer_self.app.blockchain.blocks[mb].block.id+'</a></td><td><a href="/explorer/block?hash='+explorer_self.app.blockchain.blocks[mb].hash('hex')+'">'+explorer_self.app.blockchain.blocks[mb].hash()+'</a></td><td>'+explorer_self.app.blockchain.blocks[mb].block.prevhash.substring(0,25)+'...</td>';
    } else {
      html += '<td></td><td><a href="/explorer/block?hash='+explorer_self.app.blockchain.blocks[mb].hash('hex')+'">'+explorer_self.app.blockchain.blocks[mb].block.id+'</td><td><a href="/explorer/block?bid='+explorer_self.app.blockchain.blocks[mb].block.id+'">'+explorer_self.app.blockchain.blocks[mb].hash()+'</a></td><td>'+explorer_self.app.blockchain.blocks[mb].block.prevhash.substring(0,25)+'...</td>';
    }
    html += '</tr>';
  }
  html += '</table>';
...
Explorer.prototype.listTransactions = function listTransactions(blk) {
  ...
  html += '<tr><td>hash</td><td>'+blk.hash('hex')+'</td></tr>';
