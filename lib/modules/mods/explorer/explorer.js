...
Explorer.prototype.webServer = function webServer(app, expressapp) {
  ...
  expressapp.get('/explorer/block', function (req, res) {
    ...
    if (bid == null && hash == null) {
      ...
      if (bid != null) {
        ...
        sql    = "SELECT * FROM blocks JOIN blocks_json WHERE blocks.block_json_id = blocks_json.id AND block_id = $id";
        params = { $id : bid }; 
	  ...
	  if (row == null) {
            ...
            res.setHeader('Content-type', 'text/html');
            res.charset = 'UTF-8';
            res.write(explorer_self.returnBlockHTML(explorer_self.app, row.block_json));
            res.end();
            return;
      ...
      if (hash != null) {
        ...
        sql    = "SELECT * FROM blocks JOIN blocks_json WHERE blocks.block_json_id = blocks_json.id AND hash = $hash";
        params = { $hash : hash }; 
  ...
  expressapp.get('/explorer/transaction', function (req, res) {
    ...
    if (tid == null && hash == null) {
      ...
      sql    = "SELECT * FROM txs JOIN txs_json WHERE txs.tx_json_id = txs_json.id AND tx_id = $id";
      params = { $id : tid }; 
  	...
  	if (row == null) {
          ...
          res.setHeader('Content-type', 'text/html');
          res.charset = 'UTF-8';
          res.write(explorer_self.returnTransactionHTML(row.tx_json));
          res.end();
          return;
...
Explorer.prototype.returnBlockHTML = function returnBlockHTML(app, blockjson) {
  ...
  var tmpblk = new saito.block(app, blockjson);
