...
function Saito(app) {
  ...
  this.supportsEmailInterface = 1;   // we have an email module
				     ...
				     // domain names
  this.isDomainRegistry = 1;
				     // DNS registry that will track
				     // a particular domain

  if (app.BROWSERIFY == 1) { this.isDomainRegistry = 0; }
...
console.log("SAITO REGISTRATION: ");

      sql = "SELECT count(*) AS count FROM mod_saito_addresses WHERE identifier = $identifier";
      params = { $identifier : (tx.transaction.msg.saito_identifier+"@"+this.domain) }
      app.storage.queryDatabase(sql, params, function(err, row) {

console.log("SAITO REGISTRATION 1: ");
console.log(params);
console.log(row);
        if (row != null) {
console.log("SAITO REGISTRATION 2: ");
...
Saito.prototype.displayUserInputForm = function displayUserInputForm(app) {
  ...
  element_to_edit_html = '<div id="saito_instructions" class="saito_instructions" style="font-size:1.2em;line-height:1.8em;line-spacing:1.8m;">The first thing you should do on the Saito network is register an address so others can contact you without the need to remember your public key. Be sure your transaction includes 5 SAITO payment. <p></p><input type="text" class="saito_identifier" id="saito_identifier" value="" /><div class="saito_domain">@'+this.domain+'</div><p style="clear:both;margin-top:0px;"> </p>Examples: david@'+this.domain+', yourname@'+this.domain+', etc. </div>';
