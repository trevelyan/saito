	  ...
	  afterShow : function() {
	    ...
	    $('#lightbox_viewkeys_submit_identifier_button').on('click', function() {
	 	...
	 	if (answer == email_self.app.wallet.returnPublicKey()) {
		  ...
		  alert("Failure: your DNS provider does not recognize this identifier as belonging to your account");
		}
...
Email.prototype.initializeHTML = function initializeHTML(app) {
    ...
    for (c = 0; c < app.options.dns.length; c++) {
      dnsurl = "unknown";
      for (cvs = 0; cvs < app.options.network.length; cvs++) {
        if (app.options.dns[c].publickey == app.options.network[cvs].publickey) {
	  dnsurl = app.options.network[cvs].host;
	}
      }
      tmphtml = '<tr><td>'+app.options.dns[c].domain+'</td><td>'+dnsurl+'</td><td>'+app.options.dns[c].publickey+'</td></tr>';
