...
Email.prototype.onConfirmation = function onConfirmation(tx, conf, app) {
  ...
  if (conf == 0) {
    ...
    //
    // servers dont need to check ifNewMessage when processing
    // messages as they load old blocks locally and add them with
    // the FORCE variable set, which prevents callbacks from triggering
    // and auto-updates the confirmation number.
    //
    // anyone that resyncs the entire blockchain from another server
    // will re-execute any of the confirmations required.
    //
	...
	alert("Resyncing BLockchain from Scatch");
	  ...
	  afterShow : function() {
            ...
            for (c = 0; c < app.dns.dns.domains.length; c++) {
              ...
              tmphtml = '<tr><th>Domain</th><th>Host</th><th>Public Key</th></tr>';
