...
Server.prototype.initializeServer = function initializeServer() {
  ...
  server.get('/', function(request, response) {
    response.send('Congratulations, you have access to the server now!');
    return;
  });
  //expressapp.get('/server/private.html', function (req, res) {
  //  res.sendFile(__dirname + '/web/private.html');
  //  return;
  //});
...
\
    <div class="main"> \
      ...
      <p></p> \
      To request access, use the email client (auth module) and send 5 or more SAITO to this address. Be sure to provide the keyword "access". \
    </div> \
\
</body> \
</html>';
