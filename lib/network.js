...
Network.prototype.initialize = function initialize() {
  ...
  for (i = 0; i < this.app.options.peers.length; i++) {
...
Network.prototype.addPeer = function addPeer(peerhost, peerport) {
  ...
  if (this.app.options.server.host == peerhost && this.app.BROWSER == 0) { return; }

		
  console.log("\n\nPEER ADDING: "+peerhost);
  console.log(this.app.options.server);
      
	// first we check to make sure we are not double-adding
  for (bnm = 0; bnm < this.peers.length; bnm++) {
...
Network.prototype.returnNetwork = function returnNetwork() {
  peerarray = [];
  for (ib = 0; ib < this.peers.length; ib++) {
